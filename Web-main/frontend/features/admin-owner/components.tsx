...
export function ChartCard({ title, series }: { title: string; series?: { label: string; value: number }[] }) {
...
export function DashboardPage({ scope }: { scope: 'admin' | 'owner' }) {
  ...
  const paymentSeries = summary?.payments_by_status
    ...
    : [];
  return <div className="space-y-6 sm:space-y-8"><PageHeader title={scope === 'admin' ? 'Admin Dashboard' : 'Owner Dashboard'} description="Metrik langsung dari endpoint laporan (30 hari terakhir)." />
