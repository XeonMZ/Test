...
export default function InstallPage() {
  ...
  useEffect(() => {
    http
      ...
      .then((r) => {
        setStatus(r.data);
        if (r.data?.diagnostic) setError('Diagnostik server: ' + r.data.diagnostic);
      })
      .catch((e) => setError(extractApiError(e, 'Tidak dapat memeriksa status installer.')));
  }, []);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (data?.errors) {
        // Laravel validation bag — show the first concrete field error so the
        // user knows exactly what to fix.
        const first = Object.values(data.errors)[0];
        setError(Array.isArray(first) ? String(first[0]) : String(first));
      } else {
        setError(extractApiError(e, 'Instalasi gagal. Periksa kembali isian Anda.'));
      }
    } finally {
      setLoading(false);
    }
  ...
  if (status?.installed || done)
    return (
      <main className="min-h-screen bg-slate-950 px-4 py-16 text-white">
        <section className="mx-auto max-w-2xl rounded-2xl bg-white p-10 text-center text-slate-950 shadow-soft">
          ...
          <a
            className="mt-8 inline-flex rounded-md bg-primary px-6 py-3 font-semibold text-white"
            href="/login"
          >
            Go to login
          </a>
  ...
  return (
    <main className="min-h-screen bg-slate-100 px-4 py-8">
      <form
        onSubmit={submit}
        className="mx-auto max-w-5xl rounded-2xl bg-white p-6 shadow-soft md:p-10"
      >
        <p className="text-sm font-bold uppercase tracking-button text-primary">
          SJT First-Time Setup
        </p>
