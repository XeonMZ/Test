...
function LoginForm() {
  ...
  // Sign-in is blocked until the user confirms the legal terms.
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    if (!acceptedTerms) {
      setError('Kamu harus menyetujui Syarat & Ketentuan serta Kebijakan Privasi untuk masuk.');
      return;
    }
    ...
    <main className="flex min-h-screen items-center justify-center bg-cloud px-4 py-12 dark:bg-ink-deep">
      <section className="w-full max-w-md rounded-md bg-canvas p-8 shadow-soft dark:bg-ink sm:p-10">
        ...
        <form className="mt-8 space-y-4" onSubmit={handleSubmit}>
          ...
          <LegalConsent id="login-accept-terms" checked={acceptedTerms} onChange={setAcceptedTerms}>
