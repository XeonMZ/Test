...
export function BottomSheet({ children, title }: { children: ReactNode; title: string }) {
  ...
  return (
    <section
      aria-label={title}
      className="fixed inset-x-0 bottom-0 z-30 mx-auto max-h-[62vh] w-full max-w-3xl rounded-t-3xl border border-b-0 border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-950 lg:static lg:max-h-none lg:rounded-3xl lg:border-b lg:shadow-none"
    >
      <button
        type="button"
  ...
  return (
    <div className="space-y-4 pb-[58vh] lg:pb-0">
      ...
      <div className="relative">
        ...
        <button onClick={() => query.refetch()} aria-label="Refresh manifest" className="absolute right-3 top-3 grid h-11 w-11 place-items-center rounded-2xl bg-white/95 text-slate-700 shadow-md dark:bg-slate-900/95 dark:text-slate-200">
      ...
      <BottomSheet title={`Manifest — ${remaining} penumpang belum diantar`}>
        <div className="space-y-3">
          <p className="text-xs font-extrabold uppercase tracking-wide text-slate-500">Penumpang ({data.bookings.length})</p>
              ...
              actions={
                active ? (
                  !b.picked_up_at ? (
                    ...
                    <button onClick={() => dropoffMutation.mutate(b.id)} disabled={dropoffMutation.isPending} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-primary px-3 text-xs font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">
          ...
          <p className="pt-2 text-xs font-extrabold uppercase tracking-wide text-slate-500">Paket / Jastip ({data.jastip.length})</p>
              ...
              actions={
                active ? (
                  j.status !== 'picked_up' && !j.picked_up_at ? (
                    ...
                    <button onClick={() => jastipMutation.mutate({ id: j.id, status: 'delivered' })} disabled={jastipMutation.isPending} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-primary px-3 text-xs font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">
