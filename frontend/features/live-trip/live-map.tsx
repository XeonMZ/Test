...
export function LiveMap({ points, path, center, className }: { points: MapPoint[]; path: LatLng[]; center?: LatLng | null; className?: string }) {
  ...
  if (error) {
    return (
      <div className={`grid place-items-center rounded-2xl bg-slate-100 p-6 text-center dark:bg-slate-900 ${className ?? ''}`}>
  ...
  return (
    <div className={`relative overflow-hidden rounded-2xl bg-slate-100 dark:bg-slate-900 ${className ?? ''}`}>
      ...
      {config.provider === 'beta' ? (
        <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex justify-center pt-2">
          <span className="rounded-2xl bg-amber-500 px-4 py-1.5 text-sm font-extrabold uppercase tracking-button text-white shadow-lg">⚠ SIMULATION MODE</span>
        </div>
      ) : null}
