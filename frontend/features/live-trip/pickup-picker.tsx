...
export function PickupPicker({ title, value, onChange, accent }: { title: string; value: PickedLocation; onChange: (v: PickedLocation) => void; accent: 'pickup' | 'drop' }) {
  ...
  const color = accent === 'pickup' ? '#024ad8' : '#dc2626';

  const fieldInput = 'min-h-11 w-full rounded-md border border-steel bg-canvas px-4 text-sm text-ink outline-none transition-colors placeholder:text-graphite focus:border-primary dark:border-ink-soft dark:bg-ink dark:text-slate-100';
      ...
      {!open ? (
        ...
        <div className="space-y-2 rounded-2xl border border-slate-200 p-3 dark:border-slate-700">
          {config.provider === 'beta' ? (
            <p className="rounded-xl bg-amber-100 px-3 py-1.5 text-xs font-extrabold uppercase tracking-button text-amber-800 dark:bg-amber-950/60 dark:text-amber-200">⚠ Simulation Mode — peta & pencarian bersifat simulasi</p>
          ) : null}
          <form className="flex items-center gap-2" onSubmit={(e) => { e.preventDefault(); void runSearch(); }}>
            ...
            <button type="submit" disabled={searching} title="Cari alamat" aria-label="Cari alamat" className="grid h-11 w-11 shrink-0 place-items-center rounded-md bg-slate-900 text-white transition hover:bg-slate-800 disabled:opacity-60 dark:bg-slate-800">
              {searching ? <Loader2 size={15} className="animate-spin" /> : <Search size={15} />}
            </button>
            <button type="button" onClick={useCurrentLocation} title="Gunakan lokasi saat ini" aria-label="Gunakan lokasi saat ini" className="grid h-11 w-11 shrink-0 place-items-center rounded-lg bg-primary/10 text-primary transition hover:bg-primary/20"><Crosshair size={17} /></button>
            <button type="button" onClick={() => setOpen(false)} aria-label="Tutup peta" className="grid h-11 w-11 shrink-0 place-items-center rounded-md border border-slate-200 text-slate-500 dark:border-slate-700"><X size={15} /></button>
          </form>
          {results.length > 0 ? (
