  ...
  return (
    <div className="space-y-4 pb-[58vh] lg:pb-0">
      <PageHeader
        ...
        actions={
          <Link href={backHref} className="inline-flex min-h-11 items-center gap-2 rounded-2xl border border-slate-200 px-4 text-sm font-extrabold text-slate-700 transition hover:border-primary hover:text-primary dark:border-slate-800 dark:text-slate-200">
      ...
      <div className="relative">
        ...
        <button onClick={() => query.refetch()} aria-label="Refresh" className="absolute right-3 top-3 grid h-11 w-11 place-items-center rounded-2xl bg-white/95 text-slate-700 shadow-md dark:bg-slate-900/95 dark:text-slate-200">
      ...
      <BottomSheet title={`Manifest — ${data.bookings.length} penumpang · ${data.jastip.length} paket`}>
        <div className="space-y-3">
          <p className="text-xs font-extrabold uppercase tracking-wide text-slate-500">Manifest Customer ({data.bookings.length})</p>
          ...
          <p className="pt-2 text-xs font-extrabold uppercase tracking-wide text-slate-500">Manifest Paket ({data.jastip.length})</p>
      ...
      <BottomSheet title="Info perjalanan">
        <div className="flex flex-wrap items-center gap-4">
          {data.driver?.photo ? (
            ...
            <span className="grid h-16 w-16 place-items-center rounded-2xl bg-primary/10 text-primary"><UserRound size={26} /></span>
          )}
          ...
          {data.eta_minutes != null ? (
            <div className="rounded-2xl bg-primary/10 px-4 py-2.5 text-center">
              <p className="flex items-center gap-1.5 text-lg font-extrabold text-primary"><Clock3 size={16} /> ± {data.eta_minutes} mnt</p>
              <p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Perkiraan tiba</p>
            </div>
          ) : null}
        </div>
