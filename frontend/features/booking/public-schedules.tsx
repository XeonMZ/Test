  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      <PageHeader
        ...
        actions={
          !isAuthenticated ? (
            <Link href="/login" className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-4 text-sm font-semibold uppercase tracking-button text-white transition-colors hover:bg-primary-deep">
      ...
      <AppCard>
        ...
        >
          <label className="block">
            <span className="mb-2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-button text-graphite"><MapPin size={15} /> Kota asal</span>
          ...
          <label className="block">
            <span className="mb-2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-button text-graphite"><MapPin size={15} /> Kota tujuan</span>
          ...
          <label className="block">
            <span className="mb-2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-button text-graphite"><CalendarDays size={15} /> Tanggal</span>
          ...
          <button type="submit" className="mt-auto inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-primary px-5 text-sm font-semibold uppercase tracking-button text-white transition-colors hover:bg-primary-deep">
            <Search size={16} /> Cari
          </button>
        </form>
      </AppCard>
      ...
      {!submitted ? (
        ...
        <AppCard>
          ...
          <button onClick={() => schedulesQuery.refetch()} className="mt-5 inline-flex h-11 items-center gap-2 rounded-md bg-primary px-5 text-sm font-semibold uppercase tracking-button text-white transition-colors hover:bg-primary-deep"><RefreshCw size={14} /> Coba lagi</button>
        </AppCard>
        ...
        <ul className="space-y-3">
          {schedulesQuery.data!.map((item) => (
            <li key={item.uuid}>
              <AppCard className="transition-shadow duration-200 hover:shadow-float">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    <p className="flex flex-wrap items-center gap-2 font-display text-xl font-medium tracking-tight text-ink dark:text-white">
                    ...
                    <p className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-1.5 text-sm text-graphite dark:text-slate-400">
                      ...
                      <span className={clsx('inline-flex items-center gap-1.5', item.seats_available === 0 && 'text-bloom-deep')}><Users size={14} /> {item.seats_available} kursi tersisa</span>
                    </p>
                  </div>
                  <div className="flex shrink-0 items-center gap-4 sm:flex-col sm:items-end">
                    <div className="text-left sm:text-right">
                      <p className="font-display text-xl font-medium text-primary">{formatIDR(item.base_fare)}</p>
                      <p className="text-xs font-semibold uppercase tracking-button text-graphite">per kursi</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => goBook(item)}
                      disabled={item.seats_available === 0}
                      className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-5 text-sm font-semibold uppercase tracking-button text-white transition-colors hover:bg-primary-deep disabled:cursor-not-allowed disabled:bg-steel"
                    >
      ...
      {!isAuthenticated ? (
        <p className="text-center text-sm text-graphite">
          Belum punya akun? <Link href="/register" className="font-semibold text-primary hover:underline">Daftar gratis</Link> untuk mulai memesan.
        </p>
      ) : null}
    </div>
  );
