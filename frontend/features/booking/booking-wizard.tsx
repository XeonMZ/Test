...
export function BookingWizard() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      {/* Stepper */}
      <ol className="flex items-center gap-2 overflow-x-auto rounded-3xl border border-slate-200 bg-white/90 p-3 dark:border-slate-800 dark:bg-slate-900/90" aria-label="Langkah pemesanan">
            ...
            <li key={label} className="flex min-w-0 flex-1 items-center gap-2">
              <span
                className={clsx(
                  ...
                  state === 'active' && 'bg-primary text-white shadow-md shadow-primary/30',
      ...
      {formError ? (
        <p role="alert" className="rounded-2xl bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700 ring-1 ring-rose-200 dark:bg-rose-950/40 dark:text-rose-300 dark:ring-rose-900">
          {formError}
        </p>
      ) : null}
              ...
              {schedulesQuery.data!.map((item) => (
                <li key={item.uuid}>
                  <button
                    ...
                    className="group w-full rounded-3xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-primary hover:shadow-md disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-800 dark:bg-slate-900 sm:p-6"
                  >
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
          <AppCard>
            ...
            <div className="mt-6 space-y-4">
              {selectedSeats.map((seat, index) => (
                <fieldset key={seat.id} className="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
                  <legend className="px-2 text-xs font-extrabold uppercase tracking-widest text-primary">Kursi {seat.seat_number}</legend>
            ...
            <div className="mt-6 rounded-2xl border border-slate-200 p-5 dark:border-slate-800">
              ...
              <p className="mt-1 text-xs font-semibold text-slate-500">Tempel koordinat dari Google Maps (klik lokasi → salin angka lat, lng). Pastikan sesuai arah perjalanan.</p>
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <div className="space-y-2">
                  <input value={pickup.label} onChange={(e) => setPickup({ ...pickup, label: e.target.value })} placeholder="Nama titik jemput" className={fieldInput} />
                  <div className="flex gap-2">
                    <input value={pickup.lat} onChange={(e) => setPickup({ ...pickup, lat: e.target.value })} placeholder="Lat jemput" className={fieldInput} inputMode="decimal" />
                    <input value={pickup.lng} onChange={(e) => setPickup({ ...pickup, lng: e.target.value })} placeholder="Lng jemput" className={fieldInput} inputMode="decimal" />
                  </div>
                </div>
                <div className="space-y-2">
                  <input value={drop.label} onChange={(e) => setDrop({ ...drop, label: e.target.value })} placeholder="Nama titik turun" className={fieldInput} />
                  <div className="flex gap-2">
                    <input value={drop.lat} onChange={(e) => setDrop({ ...drop, lat: e.target.value })} placeholder="Lat turun" className={fieldInput} inputMode="decimal" />
                    <input value={drop.lng} onChange={(e) => setDrop({ ...drop, lng: e.target.value })} placeholder="Lng turun" className={fieldInput} inputMode="decimal" />
                  </div>
                </div>
              </div>
              <a href="https://www.google.com/maps" target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center gap-1.5 text-xs font-extrabold text-primary hover:underline">Buka Google Maps untuk ambil koordinat →</a>
            </div>
            ...
            <div className="mt-4 rounded-2xl border border-slate-200 p-5 dark:border-slate-800">
              ...
              <div className="mt-3 flex gap-2">
                ...
                <button type="button" onClick={applyPromo} className="min-h-11 rounded-2xl bg-primary px-5 text-sm font-extrabold text-white transition hover:bg-primary/90">Pakai</button>
              </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
          ...
          <SummaryPanel schedule={schedule} seats={selectedSeats} total={totalAmount}>
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => bookingMutation.mutate()} disabled={bookingMutation.isPending} className={primaryButton}>
...
const fieldInput =
  'min-h-11 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-900 shadow-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100';
const primaryButton =
  'inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-2xl bg-primary px-5 text-sm font-extrabold text-white shadow-sm shadow-primary/20 transition hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-60';

function BackButton({ onClick, disabled }: { onClick: () => void; disabled?: boolean }) {
  return (
    <button type="button" onClick={onClick} disabled={disabled} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border border-slate-200 px-4 text-sm font-extrabold text-slate-700 transition hover:border-primary hover:text-primary disabled:opacity-50 dark:border-slate-800 dark:text-slate-200">
  ...
  return (
    <AppCard className="h-fit lg:sticky lg:top-24">
      ...
      <dl className="mt-4 space-y-3 text-sm">
        ...
        <div className="flex justify-between gap-3 border-t border-dashed border-slate-200 pt-3 dark:border-slate-800"><dt className="font-extrabold text-slate-700 dark:text-slate-200">Total</dt><dd className="text-right font-display text-xl font-extrabold text-primary">{formatIDR(total)}</dd></div>
      </dl>
