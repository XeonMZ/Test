  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <>
            <Link href="/booking" className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-primary-deep">
      ...
      <div className="flex flex-wrap gap-2">
        {FILTERS.map((option) => (
          <button
            ...
            className={`min-h-10 rounded-md px-4 text-sm font-semibold uppercase tracking-button transition ${filter === option.id ? 'bg-primary text-white' : 'border border-slate-200 text-slate-600 hover:border-primary hover:text-primary dark:border-slate-800 dark:text-slate-300'}`}
          >
            {option.label}
          </button>
        ))}
      ...
      <AppCard className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <p className="font-display text-lg font-medium text-slate-950 dark:text-white">
          ...
          <p className="mt-1 font-display text-lg font-medium text-primary">{formatIDR(booking.amount)}</p>
        </div>

        <div className="flex shrink-0 flex-wrap gap-2">
          {unpaid ? (
            <Link href={`/payment?booking=${booking.uuid}`} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-amber-600 px-4 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-amber-700">
              Bayar sekarang <ArrowRight size={15} />
            </Link>
          ) : null}
          {onTrip ? (
            <Link href={`/customer/track/${booking.uuid}`} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-emerald-600 px-4 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-emerald-700">
              🟢 Track Driver
            </Link>
          ) : null}
          {hasTicket && ticketUuid ? (
            <Link href={`/customer/tickets/${ticketUuid}`} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-4 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-primary-deep">
              Lihat tiket <ArrowRight size={15} />
            </Link>
          ) : null}
          <Link href={`/customer/bookings/${booking.uuid}`} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-slate-200 px-4 text-sm font-semibold uppercase tracking-button text-slate-700 transition hover:border-primary hover:text-primary dark:border-slate-800 dark:text-slate-200">
            Detail
          </Link>
        </div>
      </AppCard>
