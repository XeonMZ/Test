...
function PackageCard({ p, onBook }: { p: TourPackageRow; onBook: (p: TourPackageRow) => void }) {
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition hover:shadow-md dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-4">
        ...
        <button onClick={() => onBook(p)} className="mt-3 min-h-10 w-full rounded-md bg-primary text-xs font-semibold uppercase tracking-button text-white transition hover:bg-primary-deep">Pesan Paket</button>
      </div>
    </article>
  );
}
