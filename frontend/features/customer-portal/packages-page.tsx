  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm transition hover:shadow-md dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-4">
        ...
        <button onClick={() => onBook(p)} className="mt-3 min-h-10 w-full rounded-md bg-primary text-xs font-extrabold text-white transition hover:bg-primary/90">Pesan Paket</button>
      </div>
    </article>
  );
}
            ...
            {myBookings.data!.map((b: PackageBookingSummary) => (
              <li key={b.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
                ...
                <div className="flex gap-2">
                  {b.status === 'waiting_payment' ? <button onClick={() => payMutation.mutate(b.uuid)} disabled={payMutation.isPending} className="min-h-9 rounded-md bg-primary px-3 text-xs font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">Bayar Online</button> : null}
          ...
          <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-xl dark:bg-slate-900" onClick={(e) => e.stopPropagation()}>
            ...
            {instructions ? (
              <div className="mt-4 space-y-3">
                ...
                {lastBookingUuid ? <button onClick={() => payMutation.mutate(lastBookingUuid)} disabled={payMutation.isPending} className="min-h-11 w-full rounded-md bg-primary text-sm font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">{payMutation.isPending ? 'Membuka pembayaran…' : '💳 Bayar Online Sekarang'}</button> : null}
                <button onClick={() => setBooking(null)} className="min-h-11 w-full rounded-md border border-slate-200 text-sm font-extrabold text-slate-600 dark:border-slate-800 dark:text-slate-300">Nanti — cek "Booking Paket Saya"</button>
              </div>
            ) : (
              <form className="mt-4 space-y-3" onSubmit={(e) => { e.preventDefault(); createMutation.mutate(); }}>
                ...
                <div className="flex gap-2">
                  <button type="submit" disabled={!isCustomer || createMutation.isPending || !acceptedPolicies} className="min-h-11 flex-1 rounded-md bg-primary text-sm font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">{createMutation.isPending ? 'Memproses…' : 'Buat Booking'}</button>
                  <button type="button" onClick={() => setBooking(null)} className="min-h-11 rounded-md border border-slate-200 px-4 text-sm font-extrabold text-slate-600 dark:border-slate-800 dark:text-slate-300">Tutup</button>
                </div>
              </form>
            )}
          </div>
