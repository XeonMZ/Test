  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <Link href="/customer/bookings" className="inline-flex min-h-11 items-center gap-2 rounded-md border border-slate-200 px-4 text-sm font-semibold uppercase tracking-button text-slate-700 transition hover:border-primary hover:text-primary dark:border-slate-800 dark:text-slate-200">
          ...
          <aside className="h-fit space-y-4 lg:sticky lg:top-24">
            <AppCard>
              ...
              {UNPAID.includes(booking.status) ? (
                <Link href={`/payment?booking=${booking.uuid}`} className="mt-5 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-md bg-primary px-5 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-primary-deep">
                  Bayar sekarang <ArrowRight size={15} />
                </Link>
              ) : null}

              {ticket?.uuid ? (
                <Link href={`/customer/tickets/${ticket.uuid}`} className="mt-3 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-md border border-slate-200 px-5 text-sm font-semibold uppercase tracking-button text-slate-700 transition hover:border-primary hover:text-primary dark:border-slate-800 dark:text-slate-200">
                ...
                {confirming ? (
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => cancelMutation.mutate()} disabled={cancelMutation.isPending} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-md bg-rose-600 px-4 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-rose-700 disabled:opacity-60">
                        {cancelMutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <XCircle size={15} />} Ya, batalkan
                      </button>
                      <button onClick={() => setConfirming(false)} disabled={cancelMutation.isPending} className="inline-flex min-h-11 items-center justify-center rounded-md border border-slate-200 px-4 text-sm font-semibold uppercase tracking-button text-slate-700 dark:border-slate-800 dark:text-slate-200">
                        Kembali
                      </button>
                    </div>
                  </div>
                ) : (
                  <button onClick={() => setConfirming(true)} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-md border border-rose-200 px-5 text-sm font-semibold uppercase tracking-button text-rose-700 transition hover:bg-rose-50 dark:border-rose-900 dark:text-rose-300 dark:hover:bg-rose-950/40">
