...
function PromoCard({ promo }: { promo: Promo }) {
  return (
    <AppCard className="relative overflow-hidden transition-shadow duration-200 group-hover:shadow-float">
      <div className="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-primary/10" aria-hidden="true" />
      <span className="grid h-11 w-11 place-items-center rounded-lg bg-primary/10 text-primary"><BadgePercent size={20} /></span>
      <h3 className="mt-4 font-display text-xl font-medium text-slate-950 dark:text-white">{promo.name}</h3>
      <p className="mt-1 font-mono text-xs font-extrabold tracking-widest text-primary">{promo.code}</p>
      <p className="mt-3 font-display text-2xl font-medium text-emerald-600">- {formatIDR(promo.amount)}</p>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Detail Promo"
        actions={
          <Link href="/customer/promo" className="inline-flex min-h-11 items-center gap-2 rounded-md border border-slate-200 px-4 text-sm font-semibold uppercase tracking-button text-slate-700 transition hover:border-primary hover:text-primary dark:border-slate-800 dark:text-slate-200">
      ...
      {query.isLoading ? (
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
          <AppCard>
            <span className="grid h-12 w-12 place-items-center rounded-lg bg-primary/10 text-primary"><TicketPercent size={22} /></span>
            <h2 className="mt-4 font-display text-2xl font-medium text-slate-950 dark:text-white">{promo.name}</h2>
            <p className="mt-2 font-display text-3xl font-medium text-emerald-600">Potongan {formatIDR(promo.amount)}</p>
            <dl className="mt-6 grid gap-4 sm:grid-cols-2">
              <div className="rounded-md bg-slate-50 p-4 dark:bg-slate-950/60">
                <dt className="text-xs font-extrabold uppercase tracking-buttonst text-slate-400">Mulai</dt>
                <dd className="mt-1 font-bold text-slate-900 dark:text-slate-100">{formatDateTime(promo.starts_at)}</dd>
              </div>
              <div className="rounded-md bg-slate-50 p-4 dark:bg-slate-950/60">
                <dt className="text-xs font-extrabold uppercase tracking-buttonst text-slate-400">Berakhir</dt>
            ...
            {(promo.vouchers?.length ?? 0) === 0 ? (
              ...
              <ul className="mt-5 space-y-2">
                {promo.vouchers!.map((voucher) => (
                  <li key={voucher.id} className="flex items-center justify-between gap-3 rounded-md border border-dashed border-primary/40 bg-primary/5 px-4 py-3">
                    <span className="font-mono text-sm font-extrabold tracking-widest text-primary">{voucher.code}</span>
                    <button onClick={() => copy(voucher.code)} className="inline-flex items-center gap-1 rounded-md bg-primary px-3 py-1.5 text-xs font-semibold uppercase tracking-button text-white transition hover:bg-primary-deep">
                      <Copy size={12} /> Salin
                    </button>
                  </li>
                ))}
    ...
    <div className="space-y-6">
      ...
      {query.isLoading ? (
        ...
        <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
          <div className={`relative overflow-hidden rounded-2xl bg-gradient-to-br p-8 text-white shadow-lg ${LEVEL_STYLES[data.membership.level] ?? LEVEL_STYLES.bronze}`}>
            <Award size={30} className="opacity-90" />
            <p className="mt-6 text-xs font-extrabold uppercase tracking-button opacity-80">SJT Member</p>
            <p className="mt-1 font-display text-3xl font-medium capitalize">{data.membership.level}</p>
            <div className="mt-8 flex items-end justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-buttonst opacity-70">Poin</p>
                <p className="font-display text-2xl font-medium">{data.membership.points.toLocaleString('id-ID')}</p>
              </div>
              ...
              {trackable.map((booking) => (
                <li key={booking.uuid}>
                  <button
                    ...
                    className={`w-full rounded-2xl border p-4 text-left transition ${activeUuid === booking.uuid ? 'border-primary bg-primary/5' : 'border-slate-200 hover:border-primary/50 dark:border-slate-800'}`}
                  >
            ...
            {trackingQuery.isLoading ? (
              ...
              <div className="mt-5 space-y-4">
                <div className="flex flex-wrap items-center gap-3">
                  <span className="grid h-12 w-12 place-items-center rounded-lg bg-primary/10 text-primary"><Bus size={22} /></span>
                  <div>
                ...
                {tracking.driver ? (
                  <div className="rounded-2xl border border-slate-200 p-5 dark:border-slate-800">
                    <p className="text-xs font-bold uppercase tracking-buttonst text-slate-400">Driver Anda</p>
                    ...
                    <dl className="mt-4 grid gap-3 sm:grid-cols-3">
                      <div><dt className="text-xs font-bold uppercase tracking-buttonst text-slate-400">No. HP</dt><dd className="mt-1 font-bold text-slate-900 dark:text-slate-100">{tracking.driver.phone ?? '—'}</dd></div>
                      <div><dt className="text-xs font-bold uppercase tracking-buttonst text-slate-400">Plat</dt><dd className="mt-1 font-mono font-bold text-slate-900 dark:text-slate-100">{tracking.driver.vehicle_plate ?? '—'}</dd></div>
                      <div><dt className="text-xs font-bold uppercase tracking-buttonst text-slate-400">Estimasi tiba</dt><dd className="mt-1 font-bold text-slate-900 dark:text-slate-100">{tracking.eta_minutes !== null ? `${tracking.eta_minutes} mnt` : '—'}</dd></div>
                    </dl>
                    {tracking.driver.phone ? (
                      <a href={`https://wa.me/${(tracking.driver.phone).replace(/[^0-9]/g, '').replace(/^0/, '62')}`} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex items-center gap-2 rounded-md bg-emerald-600 px-4 py-2 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-emerald-700">
                ...
                {tracking.location ? (
                  <div className="rounded-2xl border border-slate-200 p-5 dark:border-slate-800">
                    ...
                    <dl className="mt-3 grid gap-3 sm:grid-cols-3">
                      <div><dt className="text-xs font-bold uppercase tracking-buttonst text-slate-400">Koordinat</dt><dd className="mt-1 font-mono text-sm font-bold text-slate-900 dark:text-slate-100">{tracking.location.latitude.toFixed(5)}, {tracking.location.longitude.toFixed(5)}</dd></div>
                      <div><dt className="text-xs font-bold uppercase tracking-buttonst text-slate-400">Kecepatan</dt><dd className="mt-1 font-bold text-slate-900 dark:text-slate-100">{tracking.location.speed !== null ? `${Math.round(tracking.location.speed)} km/j` : '—'}</dd></div>
                      <div><dt className="text-xs font-bold uppercase tracking-buttonst text-slate-400">Diperbarui</dt><dd className="mt-1 font-bold text-slate-900 dark:text-slate-100">{formatDateTime(tracking.location.recorded_at)}</dd></div>
                    </dl>
                    <a
                      href={`https://www.google.com/maps?q=${tracking.location.latitude},${tracking.location.longitude}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="mt-4 inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-primary-deep"
                    >
