          ...
          {tickets.map((ticket) => (
            <li key={ticket.uuid}>
              <Link href={`/customer/tickets/${ticket.uuid}`} className="group block">
                <AppCard className="transition-shadow duration-200 group-hover:shadow-float">
                  <div className="flex items-start justify-between gap-3">
                    <span className="grid h-11 w-11 shrink-0 place-items-center rounded-lg bg-primary/10 text-primary transition group-hover:bg-primary group-hover:text-white">
                      <QrCode size={20} />
                    </span>
