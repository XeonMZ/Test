  ...
  return (
    <div className="mx-auto grid w-full max-w-4xl gap-6 lg:grid-cols-[1fr_340px]">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-8">
        <p className="text-xs font-extrabold uppercase tracking-[0.25em] text-primary">Pembayaran</p>
        <h1 className="mt-3 font-display text-2xl font-extrabold text-slate-950 dark:text-white sm:text-3xl">Pilih metode pembayaran</h1>
        ...
        <button
          ...
          className="mt-6 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-primary px-6 text-sm font-extrabold text-white shadow-sm shadow-primary/20 transition hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-60"
        >
      ...
      <aside className="h-fit rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 lg:sticky lg:top-24">
        <div className="rounded-2xl bg-amber-50 p-4 text-center ring-1 ring-amber-200 dark:bg-amber-950/40 dark:ring-amber-900">
          <p className="text-xs font-extrabold uppercase tracking-widest text-amber-700 dark:text-amber-300">Sisa waktu pembayaran</p>
          <p className="mt-1 font-display text-3xl font-extrabold tabular-nums text-amber-700 dark:text-amber-300" aria-live="polite">{formatCountdown(remaining)}</p>
        </div>
        <dl className="mt-5 space-y-3 text-sm">
          ...
          <div className="flex justify-between gap-3 border-t border-dashed border-slate-200 pt-3 dark:border-slate-800">
            ...
            <dd className="font-display text-lg font-extrabold text-primary">{formatIDR(booking?.amount)}</dd>
          </div>
        </dl>
      </aside>
    </div>
        ...
        <>
          {gateway.redirect_url ? (
            <a
              ...
              className="mt-6 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-primary px-6 text-sm font-extrabold text-white shadow-sm shadow-primary/20 transition hover:bg-primary/90"
            >
          ...
          {gateway.va_number ? (
            <div className="mt-4 rounded-2xl bg-slate-50 px-6 py-4 ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
              <p className="text-xs font-extrabold uppercase tracking-widest text-slate-500">Nomor Virtual Account</p>
              <p className="mt-1 select-all font-display text-2xl font-extrabold tabular-nums text-slate-900 dark:text-slate-100">{gateway.va_number}</p>
            </div>
          ) : null}
          {!gateway.redirect_url && gateway.qr_string ? (
            <div className="mt-4 rounded-2xl bg-slate-50 px-6 py-4 ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
              <p className="text-xs font-extrabold uppercase tracking-widest text-slate-500">Kode QRIS</p>
          ...
          <div className="mt-6 rounded-2xl bg-amber-50 px-6 py-4 ring-1 ring-amber-200 dark:bg-amber-950/40 dark:ring-amber-900">
            <p className="text-xs font-extrabold uppercase tracking-widest text-amber-700 dark:text-amber-300">Sisa waktu</p>
            <p className="mt-1 font-display text-4xl font-extrabold tabular-nums text-amber-700 dark:text-amber-300" aria-live="polite">{formatCountdown(remaining)}</p>
          </div>
        </>
      }
...
const ctaClass = 'inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-primary px-6 text-sm font-extrabold text-white shadow-sm shadow-primary/20 transition hover:bg-primary/90';
...
function StateCard({ tone, icon: Icon, spin, title, description, action, extra }: { tone: keyof typeof tones; icon: React.ComponentType<{ size?: number; className?: string }>; spin?: boolean; title: string; description: string; action?: React.ReactNode; extra?: React.ReactNode }) {
  return (
    <div className="mx-auto grid min-h-[50vh] w-full max-w-xl place-items-center">
      <section className="w-full rounded-[2rem] border border-slate-200 bg-white p-8 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-10">
        ...
        <h1 className="mt-5 font-display text-2xl font-extrabold text-slate-950 dark:text-white sm:text-3xl">{title}</h1>
