...
export function AuditLogsPage() {
  ...
  function SortHeader({ column, children }: { column: string; children: ReactNode }) {
    ...
    return (
      <button onClick={() => toggleSort(column)} className="inline-flex items-center gap-1 font-extrabold uppercase tracking-button hover:text-primary">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          <label className="relative min-w-0 flex-1">
            ...
            <input value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="Cari aksi, objek, user…" className="min-h-11 w-full rounded-md border border-slate-200 bg-white pl-10 pr-4 text-sm font-semibold shadow-sm outline-none transition focus:border-primary dark:border-slate-800 dark:bg-slate-950" />
          </label>
        ...
        <input type="date" value={filter.from} onChange={(e) => setFilter((f) => ({ ...f, page: 1, from: e.target.value }))} aria-label="Dari tanggal" className="min-h-11 rounded-md border border-slate-200 bg-white px-3 text-sm font-bold dark:border-slate-800 dark:bg-slate-950" />
        <input type="date" value={filter.to} onChange={(e) => setFilter((f) => ({ ...f, page: 1, to: e.target.value }))} aria-label="Sampai tanggal" className="min-h-11 rounded-md border border-slate-200 bg-white px-3 text-sm font-bold dark:border-slate-800 dark:bg-slate-950" />
      </div>
                      ...
                      <tr>
                        <td colSpan={9} className="bg-slate-50 px-4 py-4 dark:bg-slate-950/60">
                          <div className="grid gap-4 lg:grid-cols-2">
                            <div>
                              <p className="text-xs font-extrabold uppercase tracking-button text-slate-400">Sebelum</p>
                            ...
                            <div>
                              <p className="text-xs font-extrabold uppercase tracking-button text-slate-400">Sesudah</p>
