      ...
      {query.isLoading ? (
        ...
        <AppCard className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead className="text-xs uppercase tracking-wide text-slate-500"><tr><th className={th}>Kode</th><th className={th}>Rute</th><th className={th}>Jarak</th><th className={th}>Durasi</th><th className={th}>Jadwal</th><th className="py-3 text-right font-extrabold">Aksi</th></tr></thead>
      ...
      <div className="flex flex-wrap gap-2" role="tablist" aria-label="Filter status jadwal">
        {SCHEDULE_TABS.map((t) => (
          <button
            ...
            className={`min-h-10 rounded-2xl px-4 text-sm font-extrabold transition ${tab === t.key ? 'bg-primary text-white shadow-sm shadow-primary/20' : 'border border-slate-200 text-slate-600 hover:border-primary hover:text-primary dark:border-slate-800 dark:text-slate-300'}`}
          >
            {t.label}
          </button>
        ))}
