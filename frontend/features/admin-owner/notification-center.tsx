...
const inputClass = 'min-h-11 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-900 shadow-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100';
const selectClass = 'min-h-11 rounded-2xl border border-slate-200 bg-white px-3 text-sm font-bold dark:border-slate-800 dark:bg-slate-950';
  ...
  return (
    <AppCard>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button onClick={() => mutation.mutate(true)} disabled={!valid || mutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-2xl bg-primary px-5 text-sm font-extrabold text-white transition hover:bg-primary/90 disabled:opacity-50">
          {mutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <Send size={15} />} Kirim Sekarang
        </button>
        <button onClick={() => mutation.mutate(false)} disabled={!valid || mutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-2xl border border-slate-200 px-5 text-sm font-extrabold text-slate-700 transition hover:border-primary hover:text-primary disabled:opacity-50 dark:border-slate-800 dark:text-slate-200">
            ...
            return (
              <AppCard key={a.id} className="overflow-hidden !p-0">
                <div className="flex flex-col gap-4 px-5 py-5 sm:flex-row sm:items-center">
                  ...
                  >
                    <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-primary/10 text-primary"><Megaphone size={18} /></span>
                  ...
                  {isDraft ? (
                    <span className="flex flex-wrap items-center gap-1.5 sm:shrink-0">
                      ...
                      <button onClick={() => sendMutation.mutate(a.id)} disabled={sendMutation.isPending} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-primary px-3 text-xs font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">
