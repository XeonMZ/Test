  ...
  return (
    <AppCard>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button onClick={() => mutation.mutate(true)} disabled={!valid || mutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-5 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-primary-deep disabled:opacity-50">
          {mutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <Send size={15} />} Kirim Sekarang
        </button>
        <button onClick={() => mutation.mutate(false)} disabled={!valid || mutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-slate-200 px-5 text-sm font-semibold uppercase tracking-button text-slate-700 transition hover:border-primary hover:text-primary disabled:opacity-50 dark:border-slate-800 dark:text-slate-200">
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        {statChips.map((chip) => (
          <div key={chip.label} className="rounded-2xl border border-slate-100 bg-white px-3 py-3 text-center dark:border-slate-800 dark:bg-slate-900">
            ...
            <p className="text-[10px] font-bold uppercase tracking-button text-slate-400">{chip.label}</p>
          </div>
        ))}
      </div>
      ...
      {by_role.length > 0 ? (
        <div className="space-y-2">
          <p className="text-xs font-extrabold uppercase tracking-button text-slate-500">Distribusi Penerima per Role</p>
          {by_role.map((row) => (
                ...
                <div className="flex flex-col gap-4 px-5 py-5 sm:flex-row sm:items-center">
                  ...
                  >
                    ...
                    <span className="flex flex-wrap items-center gap-x-5 gap-y-2 sm:justify-end">
                      <span className="text-center">
                        ...
                        <span className="block text-[10px] font-bold uppercase tracking-button text-slate-400">Target</span>
                      </span>
                      {!isDraft ? (
                        <>
                          <span className="text-center">
                            <span className="block text-sm font-extrabold text-slate-900 dark:text-slate-100">{a.recipients_count ?? a.sent_count}</span>
                            <span className="block text-[10px] font-bold uppercase tracking-button text-slate-400">Penerima</span>
                          </span>
                          <span className="text-center">
                            <span className="block text-sm font-extrabold text-emerald-600">{a.read_count ?? 0}</span>
                            <span className="block text-[10px] font-bold uppercase tracking-button text-slate-400">Dibaca</span>
                          </span>
                          <span className="text-center">
                            <span className="block text-sm font-extrabold text-amber-600">{a.unread_count ?? 0}</span>
                            <span className="block text-[10px] font-bold uppercase tracking-button text-slate-400">Belum</span>
                          </span>
                        </>
                      ) : null}
                  ...
                  {isDraft ? (
                    <span className="flex flex-wrap items-center gap-1.5 sm:shrink-0">
                      ...
                      <button onClick={() => sendMutation.mutate(a.id)} disabled={sendMutation.isPending} className="inline-flex min-h-10 items-center gap-1.5 rounded-md bg-primary px-3 text-xs font-semibold uppercase tracking-button text-white hover:bg-primary-deep disabled:opacity-60">
