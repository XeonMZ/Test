...
export function ManageFleetPage() {
  ...
  const query = useQuery({ queryKey: ['manage-fleet'], queryFn: () => adminApi.vehiclesList({}), placeholderData: keepPreviousData });
      ...
      {query.isLoading ? <Skeleton className="h-48" /> : (query.data?.data.length ?? 0) === 0 ? (
        ...
        <AppCard className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead className="text-xs uppercase tracking-wide text-slate-500"><tr><th className={th}>Kode</th><th className={th}>Merek</th><th className={th}>Plat</th><th className={th}>Kursi</th><th className={th}>Status</th></tr></thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {query.data!.data.map((v) => (
                <tr key={v.id}>
                  ...
                  <td className={td}><Badge tone={v.status === 'active' ? 'success' : v.status === 'maintenance' ? 'warning' : 'danger'}>{v.status}</Badge></td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <div className="space-y-6">
      ...
      {query.isLoading ? <Skeleton className="h-40" /> : (query.data?.data.length ?? 0) === 0 ? (
        ...
        <AppCard className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead className="text-xs uppercase tracking-wide text-slate-500"><tr><th className={th}>Nama</th><th className={th}>Email</th><th className="py-3 text-right font-extrabold">Aksi</th></tr></thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {query.data!.data.map((a) => (
                <tr key={a.id}>
                  <td className={`${td} font-bold text-slate-900 dark:text-slate-100`}>{a.name}</td>
                  <td className={td}>{a.email}</td>
                  <td className="py-3 text-right"><button onClick={() => deleteMutation.mutate(a.id)} disabled={deleteMutation.isPending} className="inline-flex min-h-9 items-center gap-1.5 rounded-xl border border-rose-200 px-3 text-xs font-extrabold text-rose-700 transition hover:bg-rose-50 dark:border-rose-900 dark:text-rose-300"><Trash2 size={13} /> Hapus</button></td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <div className="space-y-6">
      ...
      {query.isLoading ? <Skeleton className="h-40" /> : (query.data?.data.length ?? 0) === 0 ? (
        ...
        <AppCard className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead className="text-xs uppercase tracking-wide text-slate-500"><tr><th className={th}>Nama</th><th className={th}>Email</th><th className={th}>No. SIM</th><th className={th}>Status</th></tr></thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {query.data!.data.map((d) => (
                <tr key={d.id}>
                  ...
                  <td className={td}><Badge tone={d.status === 'available' ? 'success' : d.status === 'suspended' ? 'danger' : 'warning'}>{d.status}</Badge></td>
                </tr>
              ))}
            </tbody>
          </table>
