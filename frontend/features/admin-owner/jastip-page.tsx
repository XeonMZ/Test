...
export function JastipPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {open ? (
        <AppCard>
          <SectionHeader title="Paket baru" description="Pilih titik jemput & antar lewat peta — sama seperti booking penumpang." />
          <form className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3" onSubmit={(e) => { e.preventDefault(); createMutation.mutate(); }}>
            ...
            <select required value={form.driver_id} onChange={(e) => setForm({ ...form, driver_id: e.target.value })} className={input}>
              <option value="">Pilih driver (wajib)</option>
            ...
            <div />
            {/* Map picker — the SAME component the passenger booking wizard uses (Provider Pattern: Google/OSM/Beta). */}
            <div className="sm:col-span-2 lg:col-span-3 grid gap-3 lg:grid-cols-2">
              <PickupPicker title="Titik jemput paket" accent="pickup" value={{ label: form.pickup_label, lat: form.pickup_lat, lng: form.pickup_lng }} onChange={(v) => setForm((f) => ({ ...f, pickup_label: v.label, pickup_lat: v.lat, pickup_lng: v.lng }))} />
              <PickupPicker title="Titik antar paket" accent="drop" value={{ label: form.drop_label, lat: form.drop_lat, lng: form.drop_lng }} onChange={(v) => setForm((f) => ({ ...f, drop_label: v.label, drop_lat: v.lat, drop_lng: v.lng }))} />
            </div>
      ...
      {query.isLoading ? <Skeleton className="h-48" /> : (query.data?.data.length ?? 0) === 0 ? (
        ...
        <AppCard className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead className="text-xs uppercase tracking-button text-slate-500"><tr><th className={th}>Kode</th><th className={th}>Barang</th><th className={th}>Jemput → Antar</th><th className={th}>Driver</th><th className={th}>Biaya</th><th className={th}>Status</th><th className="py-3 text-right font-extrabold">Aksi</th></tr></thead>
