        ...
        <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
          <AppCard className="text-center">
            <div className="relative mx-auto h-20 w-20">
              {(profile as Record<string, unknown>).avatar_url ? (
                ...
                <img src={(profile as Record<string, unknown>).avatar_url as string} alt={profile.name} className="h-20 w-20 rounded-3xl object-cover" />
              ) : (
                <span className="grid h-20 w-20 place-items-center rounded-3xl bg-primary/10 text-primary"><UserRound size={36} /></span>
              )}
          ...
          <AppCard>
            ...
            >
              ...
              <button type="submit" disabled={saveMutation.isPending} className="inline-flex min-h-12 w-fit items-center gap-2 rounded-md bg-primary px-6 text-sm font-extrabold text-white shadow-sm shadow-primary/20 transition hover:bg-primary/90 disabled:opacity-60">
            ...
            <div className="mt-8 border-t border-slate-100 pt-6 dark:border-slate-800">
              ...
              <form className="mt-4 grid gap-3 sm:grid-cols-3" onSubmit={(e) => { e.preventDefault(); if (pwd.password.length >= 8 && pwd.password === pwd.password_confirmation) passwordMutation.mutate(); else toast('Password baru minimal 8 karakter dan konfirmasi harus cocok.', 'error'); }}>
                ...
                <div className="flex flex-wrap gap-2 sm:col-span-3">
                  <button type="submit" disabled={passwordMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-slate-900 px-5 text-sm font-extrabold text-white hover:bg-slate-800 disabled:opacity-60 dark:bg-slate-800">
                    {passwordMutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <KeyRound size={15} />} Ubah Password
                  </button>
                  <button type="button" onClick={() => resetMutation.mutate()} disabled={resetMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-slate-200 px-4 text-sm font-extrabold text-slate-600 hover:border-primary hover:text-primary disabled:opacity-60 dark:border-slate-800 dark:text-slate-300">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {REPORT_PERIODS.map((p) => (
          <button
            ...
            className={`min-h-10 rounded-md px-4 text-sm font-extrabold transition ${filter.period === p.key ? 'bg-primary text-white shadow-sm shadow-primary/20' : 'border border-slate-200 text-slate-600 hover:border-primary hover:text-primary dark:border-slate-800 dark:text-slate-300'}`}
          >
            {p.label}
          </button>
        ))}
...
const exportBtn = 'inline-flex min-h-10 items-center gap-1.5 rounded-md border border-slate-200 px-4 text-xs font-extrabold text-slate-700 transition hover:border-primary hover:text-primary disabled:opacity-50 dark:border-slate-800 dark:text-slate-200';
