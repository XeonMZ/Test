            ...
            <div className="mt-8 border-t border-slate-100 pt-6 dark:border-slate-800">
              ...
              <form className="mt-4 grid gap-3 sm:grid-cols-3" onSubmit={(e) => { e.preventDefault(); if (pwd.password.length >= 8 && pwd.password === pwd.password_confirmation) passwordMutation.mutate(); else toast('Password baru minimal 8 karakter dan konfirmasi harus cocok.', 'error'); }}>
                ...
                <div className="flex flex-wrap gap-2 sm:col-span-3">
                  <button type="submit" disabled={passwordMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-2xl bg-slate-900 px-5 text-sm font-extrabold text-white hover:bg-slate-800 disabled:opacity-60 dark:bg-slate-800">
                    {passwordMutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <KeyRound size={15} />} Ubah Password
                  </button>
                  <button type="button" onClick={() => resetMutation.mutate()} disabled={resetMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-2xl border border-slate-200 px-4 text-sm font-extrabold text-slate-600 hover:border-primary hover:text-primary disabled:opacity-60 dark:border-slate-800 dark:text-slate-300">
