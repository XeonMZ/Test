          ...
          <AppCard>
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                <button onClick={() => save.mutate()} disabled={save.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-2xl bg-primary px-5 text-sm font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">{save.isPending ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />} Simpan</button>
                <button onClick={() => previewMutation.mutate()} disabled={previewMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-2xl border border-slate-200 px-4 text-sm font-extrabold text-slate-600 hover:border-primary hover:text-primary disabled:opacity-60 dark:border-slate-800 dark:text-slate-300">{previewMutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <Eye size={15} />} Preview</button>
                <button onClick={() => { setDraft({ subject: '', heading: '', intro: '' }); }} className="inline-flex min-h-11 items-center gap-2 rounded-2xl border border-slate-200 px-4 text-sm font-extrabold text-slate-500 hover:border-rose-300 hover:text-rose-700 dark:border-slate-800"><RotateCcw size={15} /> Kembalikan ke bawaan</button>
              </div>
