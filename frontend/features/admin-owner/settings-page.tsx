          ...
          <AppCard>
            ...
            <div className="mt-5 space-y-4">
              <div className="grid gap-2 sm:grid-cols-[220px_1fr_auto] sm:items-start">
                ...
                <button onClick={() => saveMutation.mutate({ key: WELCOME_KEYS.title, public: true })} disabled={saveMutation.isPending} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-primary px-4 text-sm font-extrabold text-white transition hover:bg-primary/90 disabled:opacity-60">
                  {saveMutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />} Simpan
                </button>
              </div>
              ...
              <div className="grid gap-2 sm:grid-cols-[220px_1fr_auto] sm:items-start">
                ...
                <button onClick={() => saveMutation.mutate({ key: WELCOME_KEYS.body, public: true })} disabled={saveMutation.isPending} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-primary px-4 text-sm font-extrabold text-white transition hover:bg-primary/90 disabled:opacity-60">
                  {saveMutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />} Simpan
                </button>
              </div>
              ...
              <div className="grid gap-2 sm:grid-cols-[220px_1fr] sm:items-start">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  {values[WELCOME_KEYS.image] ? (
                    ...
                    <img src={values[WELCOME_KEYS.image]} alt="Gambar welcome notification" className="h-24 w-40 rounded-2xl object-cover ring-1 ring-slate-200 dark:ring-slate-800" />
                  ) : (
                    <span className="grid h-24 w-40 place-items-center rounded-2xl bg-slate-100 text-xs font-bold text-slate-400 dark:bg-slate-900">Belum ada gambar</span>
                  )}
                  ...
                  <button type="button" onClick={() => fileInput.current?.click()} disabled={uploading} className="inline-flex min-h-11 items-center gap-2 rounded-2xl border border-slate-200 px-4 text-sm font-extrabold text-slate-700 transition hover:border-primary hover:text-primary disabled:opacity-60 dark:border-slate-800 dark:text-slate-200">
