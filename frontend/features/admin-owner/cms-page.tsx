...
export function CmsPage() {
  ...
  const pkgPayload = () => ({
    ...
    badge: pkgForm.badge || null, cover_path: pkgForm.cover_path || null, status: pkgForm.status as 'active' | 'inactive',
  ...
  const savePkg = useMutation({
    ...
    onSuccess: () => { toast('Paket wisata tersimpan.', 'success'); setPkgForm(emptyPkg); setPkgEdit(null); qc.invalidateQueries({ queryKey: ['cms-packages'] }); },
    ...
    <div className="space-y-6">
      ...
      <AppCard>
        ...
        <form className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4" onSubmit={(e) => { e.preventDefault(); savePkg.mutate(); }}>
          ...
          <input value={pkgForm.cover_path} onChange={(e) => setPkgForm({ ...pkgForm, cover_path: e.target.value })} placeholder="Path/URL cover" className={input} />
          ...
          <div className="flex flex-wrap items-center gap-4 sm:col-span-2 lg:col-span-4">
            ...
            <button type="submit" disabled={savePkg.isPending} className="ml-auto inline-flex min-h-11 items-center gap-2 rounded-2xl bg-primary px-5 text-sm font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">
              {savePkg.isPending ? <Loader2 size={15} className="animate-spin" /> : <Plus size={15} />} {pkgEdit ? 'Simpan' : 'Buat paket'}
            </button>
            {pkgEdit ? <button type="button" onClick={() => { setPkgEdit(null); setPkgForm(emptyPkg); }} className="min-h-11 rounded-2xl border border-slate-200 px-4 text-sm font-extrabold text-slate-600 dark:border-slate-800 dark:text-slate-300">Batal</button> : null}
          </div>
        </form>
      ...
      <AppCard>
        ...
        <form className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4" onSubmit={(e) => { e.preventDefault(); saveSec.mutate(); }}>
          ...
          <div className="flex gap-2 sm:col-span-2 lg:col-span-4">
            <button type="submit" disabled={saveSec.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-2xl bg-primary px-5 text-sm font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">
              {saveSec.isPending ? <Loader2 size={15} className="animate-spin" /> : <Plus size={15} />} {secEdit ? 'Simpan' : 'Tambah section'}
            </button>
            {secEdit ? <button type="button" onClick={() => { setSecEdit(null); setSecForm(emptySec); }} className="min-h-11 rounded-2xl border border-slate-200 px-4 text-sm font-extrabold text-slate-600 dark:border-slate-800 dark:text-slate-300">Batal</button> : null}
          </div>
        </form>
        {sections.isLoading ? <Skeleton className="mt-4 h-32" /> : sections.isError ? <EmptyState title="Gagal memuat sections" description={extractApiError(sections.error, 'Terjadi kesalahan.')} /> : secRows.length === 0 ? <EmptyState title="Belum ada konten" description="Tambahkan hero, layanan, FAQ, dan lainnya lewat form di atas." /> : (
          <ul className="mt-5 divide-y divide-slate-100 dark:divide-slate-800">
            {secRows.map((s: CmsSectionRow) => (
              <li key={s.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
                <div className="min-w-0">
                  <p className="flex items-center gap-2 text-sm font-extrabold text-slate-900 dark:text-slate-100"><span className="rounded-lg bg-primary/10 px-2 py-0.5 text-[10px] uppercase tracking-wide text-primary">{s.section_type}</span> {s.title || '(tanpa judul)'} <Badge tone={s.is_active ? 'success' : 'neutral'}>{s.is_active ? 'aktif' : 'nonaktif'}</Badge></p>
