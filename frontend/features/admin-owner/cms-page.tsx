      ...
      <AppCard>
        ...
        <form className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4" onSubmit={(e) => { e.preventDefault(); savePkg.mutate(); }}>
          ...
          <div className="flex flex-wrap items-center gap-4 sm:col-span-2 lg:col-span-4">
            ...
            <button type="submit" disabled={savePkg.isPending} className="ml-auto inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-5 text-sm font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">
              {savePkg.isPending ? <Loader2 size={15} className="animate-spin" /> : <Plus size={15} />} {pkgEdit ? 'Simpan' : 'Buat paket'}
            </button>
            {pkgEdit ? <button type="button" onClick={() => { setPkgEdit(null); setPkgForm(emptyPkg); setPkgGallery([]); }} className="min-h-11 rounded-md border border-slate-200 px-4 text-sm font-extrabold text-slate-600 dark:border-slate-800 dark:text-slate-300">Batal</button> : null}
          </div>
        </form>
      ...
      <AppCard>
        ...
        <form className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4" onSubmit={(e) => { e.preventDefault(); saveSec.mutate(); }}>
          ...
          <div className="flex gap-2 sm:col-span-2 lg:col-span-4">
            <button type="submit" disabled={saveSec.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-5 text-sm font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">
              {saveSec.isPending ? <Loader2 size={15} className="animate-spin" /> : <Plus size={15} />} {secEdit ? 'Simpan' : 'Tambah section'}
            </button>
            {secEdit ? <button type="button" onClick={() => { setSecEdit(null); setSecForm(emptySec); }} className="min-h-11 rounded-md border border-slate-200 px-4 text-sm font-extrabold text-slate-600 dark:border-slate-800 dark:text-slate-300">Batal</button> : null}
          </div>
        </form>
