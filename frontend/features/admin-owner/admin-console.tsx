        ...
        >
          <label className="relative min-w-0 flex-1">
            ...
            <input
              ...
              placeholder="Cari…"
              className="min-h-11 w-full rounded-2xl border border-slate-200 bg-white pl-10 pr-4 text-sm font-semibold shadow-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-slate-800 dark:bg-slate-950"
            />
          </label>
    ...
    >
      {query.data?.data.map((vehicle) => (
        <tr key={vehicle.id}>
          ...
          <td className={td}><Badge tone={statusTone(vehicle.status)}>{vehicle.status ?? '—'}</Badge></td>
          <td className="py-3 text-right">
            <select
    ...
    <div className="space-y-6">
      <AppCard>
        ...
        >
          ...
          <button type="submit" disabled={broadcastMutation.isPending} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-primary px-5 text-sm font-extrabold text-white transition hover:bg-primary/90 disabled:opacity-60 sm:col-span-2">
              ...
              <li key={setting.key} className="flex flex-wrap items-center gap-3 py-4">
                ...
                {editing === setting.key ? (
                  <span className="flex gap-2">
                    <button onClick={() => save(setting.key)} disabled={saveMutation.isPending} className="inline-flex min-h-9 items-center gap-1.5 rounded-xl bg-primary px-3 text-xs font-extrabold text-white transition hover:bg-primary/90 disabled:opacity-60">
