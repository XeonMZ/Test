...
function ChartCard({ title, series }: { title: string; series?: { label: string; value: number }[] }) {
  ...
  return (
    <AppCard>
      ...
      {data.length === 0 ? (
        <div className="mt-5 grid h-44 place-items-center rounded-2xl bg-slate-50 text-sm font-semibold text-slate-400 dark:bg-slate-950/60">Belum ada data</div>
      ) : (
        <div className="mt-5 flex h-44 items-end gap-2 rounded-2xl bg-slate-50 p-4 dark:bg-slate-950/60">
          {data.map((d) => (
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
          {items.map((item) => (
            <div key={item.label} className="rounded-2xl bg-slate-50 px-3 py-3 text-center dark:bg-slate-950/60">
              ...
              <p className="mt-0.5 text-[10px] font-bold uppercase tracking-button text-slate-400">{item.label}</p>
            </div>
          ))}
        </div>
      )}
