...
type Tab = 'builder' | 'content' | 'branding' | 'versions';
      ...
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <button onClick={() => publishMutation.mutate('draft')} disabled={publishMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-2xl border border-slate-200 px-4 text-sm font-extrabold text-slate-600 hover:border-primary hover:text-primary disabled:opacity-60 dark:border-slate-800 dark:text-slate-300"><Save size={15} /> Simpan Draft</button>
            <button onClick={() => publishMutation.mutate('published')} disabled={publishMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-2xl bg-primary px-5 text-sm font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">{publishMutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <Send size={15} />} Publish</button>
          </div>
        }
      />

      <div className="flex flex-wrap gap-2">
        {([['builder', 'Page Builder'], ['content', 'Konten (tabel)'], ['branding', 'Branding & SEO'], ['versions', 'Riwayat Versi']] as Array<[Tab, string]>).map(([key, label]) => (
      ...
      <div className={`grid gap-5 ${tab === 'builder' ? '' : 'xl:grid-cols-[1fr_460px]'}`}>
        ...
        <div className={tab === 'builder' ? 'hidden' : 'hidden xl:block'}>
          <AppCard className="sticky top-6">
            ...
            <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 dark:border-slate-800">
...
function BrandingEditor() {
  ...
  return (
    <AppCard>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <input type="color" value={b.primary_color ?? '#2563EB'} onChange={(e) => set({ primary_color: e.target.value })} className="h-11 w-14 rounded-xl border border-slate-200" />
            <input value={b.primary_color ?? ''} onChange={(e) => set({ primary_color: e.target.value })} placeholder="#2563EB" className={input} />
          </div>
        </div>
        <div>
      ...
      <button onClick={() => save.mutate()} disabled={save.isPending} className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-2xl bg-primary px-5 text-sm font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">
