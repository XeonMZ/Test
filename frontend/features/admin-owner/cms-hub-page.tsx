...
type Tab = 'builder' | 'content' | 'branding' | 'legal' | 'versions';
      ...
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <button onClick={() => publishMutation.mutate('draft')} disabled={publishMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-slate-200 px-4 text-sm font-extrabold text-slate-600 hover:border-primary hover:text-primary disabled:opacity-60 dark:border-slate-800 dark:text-slate-300"><Save size={15} /> Simpan Draft</button>
            <button onClick={() => publishMutation.mutate('published')} disabled={publishMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-5 text-sm font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">{publishMutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <Send size={15} />} Publish</button>
          </div>
        }
      />

      <div className="flex flex-wrap gap-2">
        {([['builder', 'Page Builder'], ['content', 'Konten (tabel)'], ['branding', 'Branding & SEO'], ['legal', 'Halaman Legal'], ['versions', 'Riwayat Versi']] as Array<[Tab, string]>).map(([key, label]) => (
...
function BrandingEditor() {
  ...
  return (
    <AppCard>
      ...
      <button onClick={() => save.mutate()} disabled={save.isPending} className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-5 text-sm font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">
