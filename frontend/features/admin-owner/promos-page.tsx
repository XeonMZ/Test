      ...
      <AppCard>
        ...
        <form className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5" onSubmit={(e) => { e.preventDefault(); saveMutation.mutate(); }}>
          ...
          <div className="flex gap-2 sm:col-span-2 lg:col-span-5">
            <button type="submit" disabled={saveMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-5 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-primary-deep disabled:opacity-60">
              {saveMutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <Plus size={15} />} {editId ? 'Simpan perubahan' : 'Buat promo'}
            </button>
            {editId ? <button type="button" onClick={() => { setEditId(null); setForm(empty); }} className="min-h-11 rounded-md border border-slate-200 px-4 text-sm font-semibold uppercase tracking-button text-slate-600 dark:border-slate-800 dark:text-slate-300">Batal</button> : null}
          </div>
        </form>
      </AppCard>
      <AppCard>
