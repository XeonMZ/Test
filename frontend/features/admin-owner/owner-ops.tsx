  ...
  return (
    <div className="space-y-6">
      ...
      <AppCard>
        ...
        <div className="mt-5">
          <button onClick={downloadFullBackup} disabled={downloadingFull} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-5 text-sm font-extrabold text-white transition hover:bg-primary/90 disabled:opacity-60">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <AppCard>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <button onClick={() => backupMutation.mutate()} disabled={backupMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-5 text-sm font-extrabold text-white transition hover:bg-primary/90 disabled:opacity-60">
              {backupMutation.isPending ? <Loader2 size={16} className="animate-spin" /> : <Archive size={16} />} Buat backup
            </button>
            <button onClick={downloadExport} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-slate-200 px-5 text-sm font-extrabold text-slate-700 transition hover:border-primary hover:text-primary dark:border-slate-800 dark:text-slate-200">
        ...
        {confirmingWipe ? (
          <div className="mt-5 space-y-3">
            ...
            <div className="flex gap-2">
              <button onClick={() => wipeDemoMutation.mutate()} disabled={wipeDemoMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-rose-600 px-5 text-sm font-extrabold text-white transition hover:bg-rose-700 disabled:opacity-60">
                {wipeDemoMutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <Trash2 size={15} />} Ya, hapus data demo
              </button>
              <button onClick={() => setConfirmingWipe(false)} className="inline-flex min-h-11 items-center rounded-md border border-slate-200 px-5 text-sm font-extrabold text-slate-700 dark:border-slate-800 dark:text-slate-200">Batal</button>
            </div>
          </div>
        ) : (
          <button onClick={() => setConfirmingWipe(true)} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-md border border-rose-200 px-5 text-sm font-extrabold text-rose-700 transition hover:bg-rose-50 dark:border-rose-900 dark:text-rose-300 dark:hover:bg-rose-950/40">
