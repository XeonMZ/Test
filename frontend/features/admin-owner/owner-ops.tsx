...
export function BackupPage() {
  ...
  async function downloadFullBackup() {
    ...
    try {
      const response = await http.get('/owner/production-readiness/backup/download', { responseType: 'blob', timeout: 300_000 });
      const url = URL.createObjectURL(response.data as Blob);
      const anchor = document.createElement('a');
      anchor.href = url;
      anchor.download = `stms-full-backup-${new Date().toISOString().slice(0, 10)}.zip`;
      anchor.click();
      URL.revokeObjectURL(url);
      toast('Full backup berhasil diunduh.', 'success');
    } catch (error) {
      toast(extractApiError(error, 'Full backup gagal.'), 'error');
    } finally {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Backup & Konfigurasi" description="Full backup (database, file upload, konfigurasi), backup konfigurasi, dan pembersihan data demo sebelum go-live." />

      <AppCard>
        <SectionHeader title="Full Backup (ZIP)" description="Satu arsip berisi database (JSON per tabel), seluruh file upload, konfigurasi & feature flags, plus panduan RESTORE.md — siap dipakai memulihkan sistem di server lain." />
        <div className="mt-5">
          <button onClick={downloadFullBackup} disabled={downloadingFull} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-5 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-primary-deep disabled:opacity-60">
            {downloadingFull ? <Loader2 size={16} className="animate-spin" /> : <Download size={16} />} {downloadingFull ? 'Menyiapkan arsip…' : 'Download Full Backup'}
          </button>
          <p className="mt-3 text-xs font-semibold text-slate-400">Dibatasi 3 unduhan per 10 menit. Untuk data besar, proses bisa memakan waktu beberapa saat.</p>
        </div>
      </AppCard>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <AppCard>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <button onClick={() => backupMutation.mutate()} disabled={backupMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-5 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-primary-deep disabled:opacity-60">
              {backupMutation.isPending ? <Loader2 size={16} className="animate-spin" /> : <Archive size={16} />} Buat backup
            </button>
            <button onClick={downloadExport} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-slate-200 px-5 text-sm font-semibold uppercase tracking-button text-slate-700 transition hover:border-primary hover:text-primary dark:border-slate-800 dark:text-slate-200">
              <Download size={16} /> Ekspor JSON
            </button>
          </div>
          {lastBackup ? (
            <pre className="mt-5 max-h-48 overflow-auto rounded-md bg-slate-950 p-4 text-xs leading-5 text-emerald-300">{JSON.stringify(lastBackup, null, 2)}</pre>
          ) : null}
        </AppCard>
        ...
        {confirmingWipe ? (
          <div className="mt-5 space-y-3">
            ...
            <div className="flex gap-2">
              <button onClick={() => wipeDemoMutation.mutate()} disabled={wipeDemoMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-rose-600 px-5 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-rose-700 disabled:opacity-60">
                {wipeDemoMutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <Trash2 size={15} />} Ya, hapus data demo
              </button>
              <button onClick={() => setConfirmingWipe(false)} className="inline-flex min-h-11 items-center rounded-md border border-slate-200 px-5 text-sm font-semibold uppercase tracking-button text-slate-700 dark:border-slate-800 dark:text-slate-200">Batal</button>
            </div>
          </div>
        ) : (
          <button onClick={() => setConfirmingWipe(true)} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-md border border-rose-200 px-5 text-sm font-semibold uppercase tracking-button text-rose-700 transition hover:bg-rose-50 dark:border-rose-900 dark:text-rose-300 dark:hover:bg-rose-950/40">
