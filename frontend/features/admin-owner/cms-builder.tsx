...
const input = 'min-h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-900 outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100';
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-extrabold text-slate-950 dark:text-white">Page Builder</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => saveDraft.mutate()} disabled={saveDraft.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-2xl border border-slate-200 px-4 text-sm font-extrabold text-slate-600 hover:border-primary hover:text-primary disabled:opacity-60 dark:border-slate-800 dark:text-slate-300">{saveDraft.isPending ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />} Draft</button>
          <button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-2xl bg-primary px-5 text-sm font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">{saveMutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <Eye size={15} />} Publish</button>
        </div>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[260px_1fr_340px]">
        ...
        <AppCard className="h-fit">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setShowPalette(true)} className="inline-flex min-h-9 items-center gap-1 rounded-xl bg-primary px-3 text-xs font-extrabold text-white hover:bg-primary/90"><Plus size={13} /> Tambah</button>
          </div>
          {blocks.length === 0 ? (
      ...
      {showPalette ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/50 p-4" onClick={() => setShowPalette(false)}>
          <div className="w-full max-w-2xl rounded-3xl bg-white p-6 shadow-xl dark:bg-slate-900" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
  ...
  if (type === 'color') {
    return (
      <div>
        <label className="mb-1 block text-xs font-extrabold text-slate-500">{label}</label>
        <div className="flex items-center gap-2">
          <input type="color" value={value || '#2563EB'} onChange={(e) => onChange(e.target.value)} className="h-10 w-12 rounded-lg border border-slate-200" />
          <input value={value} onChange={(e) => onChange(e.target.value)} placeholder="#2563EB" className={input} />
        </div>
      </div>
    );
  }
