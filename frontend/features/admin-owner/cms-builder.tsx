    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => saveDraft.mutate()} disabled={saveDraft.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-slate-200 px-4 text-sm font-extrabold text-slate-600 hover:border-primary hover:text-primary disabled:opacity-60 dark:border-slate-800 dark:text-slate-300">{saveDraft.isPending ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />} Draft</button>
          <button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-5 text-sm font-extrabold text-white hover:bg-primary/90 disabled:opacity-60">{saveMutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <Eye size={15} />} Publish</button>
        </div>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[260px_1fr_340px]">
        ...
        <AppCard className="h-fit">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setShowPalette(true)} className="inline-flex min-h-9 items-center gap-1 rounded-md bg-primary px-3 text-xs font-extrabold text-white hover:bg-primary/90"><Plus size={13} /> Tambah</button>
          </div>
          {blocks.length === 0 ? (
