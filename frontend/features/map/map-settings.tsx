      ...
      <AppCard>
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            ...
            disabled={busy}
            className={`min-h-11 rounded-md px-5 text-sm font-extrabold text-white transition disabled:opacity-60 ${resolution.beta_enabled ? 'bg-rose-600 hover:bg-rose-700' : 'bg-emerald-600 hover:bg-emerald-700'}`}
          >
      ...
      <AppCard>
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <button onClick={() => simulatorMutation.mutate('start')} disabled={simulatorMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-emerald-600 px-4 text-sm font-extrabold text-white transition hover:bg-emerald-700 disabled:opacity-60"><Play size={14} /> Jalankan</button>
            <button onClick={() => simulatorMutation.mutate('stop')} disabled={simulatorMutation.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-slate-200 px-4 text-sm font-extrabold text-slate-700 transition hover:border-rose-300 hover:text-rose-700 disabled:opacity-60 dark:border-slate-800 dark:text-slate-200"><Square size={14} /> Hentikan</button>
            <Link href="/owner/map-simulator" className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-4 text-sm font-extrabold text-white transition hover:bg-primary/90"><FlaskConical size={14} /> Buka Simulator</Link>
          </div>
        </div>
      </AppCard>
    </div>
