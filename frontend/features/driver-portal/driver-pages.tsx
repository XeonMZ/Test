...
export function DriverDashboardPage() {
  ...
  const tripMutation = useMutation({
    ...
    onSuccess: (_r, vars) => {
      toast(vars.action === 'start-trip' ? 'Perjalanan dimulai. Membuka Live Trip…' : 'Perjalanan diselesaikan.', 'success');
      if (vars.action === 'start-trip') router.push(`/driver/live?trip=${vars.tripId}`);
      refresh();
    },
  ...
  // Backend DriverShiftStateMachine states: offline | available | on_duty | on_trip | break | finished.
  const onShift = shiftStatus !== null && !['offline', 'finished'].includes(shiftStatus);
            ...
            {tripsQuery.isLoading ? (
              ...
              <ul className="mt-5 divide-y divide-slate-100 dark:divide-slate-800">
                {tripsQuery.data!.map((trip) => {
                  ...
                  const isLive = ['started', 'pickup', 'boarding', 'on_route', 'drop_off'].includes(trip.status);
                  return (
                    <li key={trip.uuid} className="flex flex-wrap items-center justify-between gap-3 py-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {isLive ? (
                          <>
                            <Link href={`/driver/live?trip=${trip.id}`} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-emerald-600 px-3 text-xs font-extrabold text-white transition hover:bg-emerald-700">
                              <MapPin size={13} /> Live Trip
                            </Link>
                            {/* Check-in is reachable ONLY while a trip is live (menu entry removed per spec). */}
                            <Link href="/driver/check-in" className="inline-flex min-h-10 items-center gap-1.5 rounded-xl border border-emerald-300 px-3 text-xs font-extrabold text-emerald-700 transition hover:bg-emerald-50 dark:border-emerald-900 dark:text-emerald-300">
                              Check-in
                            </Link>
                          </>
                        ) : null}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {(tripsQuery.data?.length ?? 0) > 1 ? (
          <select
            ...
            className="min-h-10 rounded-md border border-slate-200 bg-white px-3 text-sm font-bold dark:border-slate-800 dark:bg-slate-950"
...
const btnPrimary = 'inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-primary px-5 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-primary-deep disabled:cursor-not-allowed disabled:opacity-60';
const btnPrimarySm = 'inline-flex min-h-9 items-center gap-1.5 rounded-md bg-primary px-3 text-xs font-semibold uppercase tracking-button text-white transition hover:bg-primary-deep disabled:opacity-60';
const btnSuccess = 'inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-emerald-600 px-5 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-emerald-700 disabled:opacity-60';
...
const btnDanger = 'inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-rose-600 px-5 text-sm font-semibold uppercase tracking-button text-white transition hover:bg-rose-700 disabled:opacity-60';
const btnOutline = 'inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-slate-200 px-5 text-sm font-semibold uppercase tracking-button text-slate-700 transition hover:border-primary hover:text-primary dark:border-slate-800 dark:text-slate-200 disabled:opacity-60';
