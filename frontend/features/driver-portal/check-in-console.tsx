...
import { CheckCircle2, Hash, Loader2, QrCode, Users, UserX } from 'lucide-react';
...
const inputClass = 'min-h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 font-mono text-sm font-semibold text-slate-900 shadow-sm outline-none transition focus:border-primary dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100';
...
export function CheckInConsole() {
  ...
  return (
    <AppCard>
      <SectionHeader title="Check-in penumpang" description="Pindai QR dengan kamera atau foto, atau masukkan kode tiket manual." />
      ...
      <div className="mt-5 inline-flex rounded-2xl border border-slate-200 p-1 dark:border-slate-800" role="tablist" aria-label="Mode check-in">
        {([['scan', 'Scan QR', QrCode], ['code', 'Kode Tiket', Hash]] as const).map(([id, label, Icon]) => (
          <button
            key={id}
            role="tab"
      ...
      {verified ? (
        <div className="mt-6 rounded-md border border-emerald-200 bg-emerald-50 p-5 dark:border-emerald-900 dark:bg-emerald-950/40">
          <div className="flex flex-wrap items-center gap-3">
            <span className="grid h-11 w-11 place-items-center rounded-md bg-emerald-100 text-emerald-600 dark:bg-emerald-900/60"><QrCode size={20} /></span>
