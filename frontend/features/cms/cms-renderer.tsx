...
export function BlockView({ block }: { block: CmsBlock }) {
  switch (block.type) {
      ...
      return (
        <section className="relative px-4 pb-16 pt-20">
          <div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-[1.05fr_0.95fr]">
            <div>
              {m(block, 'eyebrow') ? <p className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-button text-primary"><Sparkles size={16} /> {m(block, 'eyebrow')}</p> : null}
              <h1 className="mt-6 font-display text-4xl font-medium leading-none tracking-tight sm:text-6xl">{m(block, 'title')}</h1>
              {m(block, 'subtitle') ? <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-600 dark:text-slate-300">{m(block, 'subtitle')}</p> : null}
              {m(block, 'cta_label') ? <a href={m(block, 'cta_link', '#')} className="mt-7 inline-flex h-11 items-center gap-2 rounded-md bg-primary px-6 text-sm font-semibold uppercase tracking-button text-white transition-colors hover:bg-primary-deep">{m(block, 'cta_label')} <ArrowRight size={18} /></a> : null}
            </div>
            {img ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={img} alt="" className="w-full rounded-2xl object-cover shadow-soft" />
            ) : <div className="grid h-64 place-items-center rounded-2xl bg-primary/10 text-primary"><Car size={56} /></div>}
          </div>
        </section>
      );
    }
    case 'banner':
      return (
        <section className="px-4 py-4">
          <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 rounded-2xl px-6 py-4 text-white shadow-soft" style={{ backgroundColor: m(block, 'bg_color', '#024ad8') }}>
        ...
        <section className="px-4 py-16">
          <div className="mx-auto grid max-w-7xl items-center gap-10 md:grid-cols-2">
            {img ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={img} alt="" className="w-full rounded-2xl object-cover shadow-soft" />
            ) : <div className="grid h-56 place-items-center rounded-2xl bg-slate-100 text-slate-400 dark:bg-slate-800">Gambar</div>}
            <div>
              <h2 className="font-display text-3xl font-medium">{m(block, 'title')}</h2>
      ...
      return (
        <section className="px-4 py-16">
          <div className="mx-auto max-w-7xl">
            {m(block, 'title') ? <h2 className="text-center font-display text-3xl font-medium">{m(block, 'title')}</h2> : null}
            <div className="mt-10 grid gap-6 md:grid-cols-3">
              {list(block, 'items').map((it, i) => {
                const Icon = ICONS[it.icon] ?? Sparkles;
                return (
                  <article key={i} className="rounded-2xl bg-white p-8 shadow-soft dark:bg-slate-900">
    ...
    case 'stats':
      return (
        <section className="px-4 py-10">
          <div className="mx-auto grid max-w-7xl gap-4 rounded-2xl bg-primary p-6 text-white shadow-soft sm:grid-cols-2 md:grid-cols-4">
            {list(block, 'items').map((it, i) => (
              <div key={i}><p className="font-display text-2xl font-medium">{it.value}</p><p className="mt-1 text-sm text-white/80">{it.label}</p></div>
            ))}
          </div>
        </section>
      );
    case 'promo': {
      ...
      return (
        <section className="px-4 py-16">
          <div className="mx-auto grid max-w-7xl items-center gap-8 rounded-2xl bg-white p-8 shadow-soft dark:bg-slate-900 md:grid-cols-2">
            <div>
              <h2 className="font-display text-3xl font-medium">{m(block, 'title')}</h2>
              <p className="mt-3 leading-8 text-slate-600 dark:text-slate-300">{m(block, 'body')}</p>
              {m(block, 'cta_label') ? <a href={m(block, 'cta_link', '#')} className="mt-6 inline-flex items-center gap-2 rounded-md bg-primary px-6 py-3 text-sm font-semibold uppercase tracking-button text-white">{m(block, 'cta_label')} <ArrowRight size={18} /></a> : null}
            </div>
            {img ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={img} alt="" className="w-full rounded-2xl object-cover" />
            ) : <div className="grid h-48 place-items-center rounded-2xl bg-amber-100 text-amber-500"><Tag size={44} /></div>}
          </div>
        </section>
      );
    }
        ...
        <section className="px-4 py-16">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {list(block, 'items').map((it, i) => (
                <figure key={i} className="rounded-2xl bg-slate-50 p-8 dark:bg-slate-900">
    ...
    case 'contact':
      return (
        <section className="px-4 py-16">
          <div className="mx-auto max-w-3xl rounded-2xl bg-white p-8 text-center shadow-soft dark:bg-slate-900">
            <h2 className="font-display text-3xl font-medium">{m(block, 'title')}</h2>
    ...
    case 'footer':
      return (
        <footer className="bg-slate-950 px-4 py-12 text-white">
          <div className="mx-auto max-w-7xl">
            <p className="font-display text-lg font-medium">{m(block, 'company')}</p>
