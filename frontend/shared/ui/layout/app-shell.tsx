  ...
  return (
    <nav aria-label="Breadcrumb">
      <ol className="flex flex-wrap items-center gap-2 text-sm font-semibold text-slate-500">
        <li><Link href="/" className="rounded transition hover:text-primary hover:underline">Home</Link></li>
        {crumbs.map((crumb, index) => {
          ...
          return (
            <li key={href} className="flex items-center gap-2">
              ...
              {isLast ? (
                <span aria-current="page" className="capitalize text-slate-700 dark:text-slate-200">{label}</span>
              ) : (
                <Link href={href} className="capitalize transition hover:text-primary hover:underline">{label}</Link>
              )}
            </li>
          );
        })}
