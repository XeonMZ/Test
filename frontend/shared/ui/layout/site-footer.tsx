...
import { fetchCompanyProfilePublic, fetchPublicSettings } from '@/services/stms';
import { DEFAULT_COMPANY_PROFILE } from '@/features/company/company-profile';
...
export function SiteFooter() {
  ...
  // Company wording is editable from the dashboard; fall back to the seeded default.
  const { data: profileRaw } = useQuery({ queryKey: ['company-profile-public'], queryFn: fetchCompanyProfilePublic, staleTime: 5 * 60_000 });
  const text = (key: keyof typeof DEFAULT_COMPANY_PROFILE, fallback: string): string => {
    const value = profileRaw?.[key];
    return typeof value === 'string' && value.trim() !== '' ? value.trim() : fallback;
  };
  const companyName = data?.company_name || text('name', DEFAULT_COMPANY_PROFILE.name);
  const legalName = text('legal_name', DEFAULT_COMPANY_PROFILE.legal_name);
  const location = text('location', DEFAULT_COMPANY_PROFILE.location);
    ...
    <footer className="relative overflow-hidden bg-ink text-on-ink">
      ...
      <div className="mx-auto max-w-7xl px-4 py-section-sm">
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_1.2fr]">
          {/* Brand */}
          <div>
            <p className="font-display text-2xl font-medium tracking-tight text-white">{companyName}</p>
            <p className="mt-1 text-sm font-medium text-primary-bright">{legalName}</p>
            ...
            <p className="mt-4 inline-flex items-center gap-2 text-sm text-slate-400">
              <MapPin size={14} className="text-primary-bright" /> {location}
            </p>
            {socials.length > 0 ? (
        ...
        <div className="mt-10 flex flex-col items-center justify-between gap-3 border-t border-ink-soft pt-6 sm:flex-row">
          <p className="text-xs text-slate-500">
            © {new Date().getFullYear()} {companyName} · {legalName}. Seluruh Hak Dilindungi Undang-Undang.
          </p>
