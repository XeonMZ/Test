...
const panel = 'border border-hairline bg-canvas text-ink shadow-soft transition-colors duration-200 dark:border-ink-soft dark:bg-ink dark:text-white';
...
export function StatsCard({ label, value, helper }: { label: string; value: string; helper?: string }) {
  return <AppCard className="group relative overflow-hidden"><div className="absolute inset-x-0 top-0 h-1 bg-primary" /><p className="text-xs font-bold uppercase tracking-button text-slate-500 dark:text-slate-400">{label}</p><p className="mt-3 font-display text-3xl font-medium tracking-tight text-ink dark:text-white sm:text-4xl">{value}</p>{helper ? <p className="mt-2 max-w-prose text-sm leading-6 text-slate-500 dark:text-slate-400">{helper}</p> : null}</AppCard>;
}

export function PageHeader({ title, description, actions }: { title: string; description?: string; actions?: ReactNode }) {
  return <header className="flex flex-col gap-4 py-1 md:flex-row md:items-end md:justify-between"><div className="min-w-0"><p className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-button text-primary"><ChevronMark /> SJT</p><h1 className="font-display text-3xl font-medium tracking-tight text-ink dark:text-white sm:text-4xl">{title}</h1>{description ? <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600 dark:text-slate-300 sm:text-base">{description}</p> : null}</div>{actions ? <div className="flex shrink-0 flex-wrap items-center gap-2">{actions}</div> : null}</header>;
}

export function SectionHeader({ title, description }: { title: string; description?: string }) {
  return <div className="space-y-1"><h2 className="font-display text-lg font-medium tracking-tight text-slate-950 dark:text-white sm:text-xl">{title}</h2>{description ? <p className="max-w-2xl text-sm leading-6 text-slate-500 dark:text-slate-400">{description}</p> : null}</div>;
}

export function SearchBar({ placeholder = 'Search SJT...', className }: { placeholder?: string; className?: string }) {
  return <label className={clsx('group flex min-h-11 w-full items-center gap-3 rounded-md border border-slate-200 bg-white px-4 text-sm shadow-sm transition duration-200 hover:border-primary/50 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 dark:border-slate-800 dark:bg-slate-950 sm:w-72', className)}><Search size={18} aria-hidden="true" className="shrink-0 text-slate-400 transition group-focus-within:text-primary" /><span className="sr-only">{placeholder}</span><input aria-label={placeholder} placeholder={placeholder} className="h-11 w-full bg-transparent text-slate-900 outline-none placeholder:text-slate-400 dark:text-slate-100" /></label>;
}
...
export function EmptyState({ title = 'No records yet', description = 'Content will appear here when available.' }) { return <AppCard className="grid place-items-center px-6 py-12 text-center"><div className="grid h-14 w-14 place-items-center rounded-lg bg-primary/10 text-primary"><Bell aria-hidden="true" /></div><h3 className="mt-5 font-display text-xl font-medium tracking-tight">{title}</h3><p className="mt-2 max-w-md text-sm leading-6 text-slate-500 dark:text-slate-400">{description}</p></AppCard>; }
...
export function ActionButton({ className, ...props }: ButtonHTMLAttributes<HTMLButtonElement>) { return <button className={clsx('inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-extrabold text-white shadow-sm shadow-primary/20 transition duration-200 hover:-translate-y-0.5 hover:bg-primary/90 disabled:translate-y-0 disabled:cursor-not-allowed disabled:opacity-60', focusRing, className)} {...props} />; }
