...
export function IntegratedResourcePage({ title, description, endpoint, queryKey, columns = [{ key: 'id', label: 'ID' }, { key: 'status', label: 'Status' }, { key: 'created_at', label: 'Created' }], allowedRoles, currentRole, realtimeTopic }: IntegratedResourcePageProps) {
  ...
  return <RoleGuard role={currentRole} allowed={allowedRoles}><PermissionGuard><div className="space-y-6 sm:space-y-8">
    ...
    <FilterBar><div onChange={(event) => { setSearch((event.target as HTMLInputElement).value); setPage(1); }}><SearchBar placeholder={`Search ${title}`} /></div><select aria-label="Filter by status" className="min-h-11 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm transition focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200" value={status} onChange={(event) => { setStatus(event.target.value); setPage(1); }}><option value="all">All statuses</option>{statuses.map((item) => <option key={item} value={item}>{item}</option>)}</select><select aria-label="Sort records" className="min-h-11 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm transition focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200" value={sortKey} onChange={(event) => setSortKey(event.target.value)}>{columns.map((column) => <option key={column.key} value={column.key}>Sort by {column.label}</option>)}</select>{realtimeTopic ? <Badge tone="success">Realtime: {realtimeTopic}</Badge> : <Badge>View only</Badge>}</FilterBar>
