...
export async function createBooking(payload: { schedule_id: number; seat_ids: number[]; passengers: PassengerInput[] }): Promise<Booking> {
...
export async function fetchOwnerAnalytics(): Promise<OwnerAnalytics> {
  const res = await http.get<ApiEnvelope<OwnerAnalytics>>('/owner/analytics');
  return res.data.data;
}
