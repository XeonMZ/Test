...
export function ManageVehicleSeatsPage() {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [vehicleId, setVehicleId] = useState<number | null>(null);
  const [spec, setSpec] = useState({ prefix: 'A', count: '12', class: 'regular' });

  const options = useFormOptions();
  const seatsQuery = useQuery({
    queryKey: ['vehicle-seats', vehicleId],
    queryFn: () => adminApi.vehicleSeats(vehicleId!),
    enabled: Boolean(vehicleId),
  });

  const generateMutation = useMutation({
    mutationFn: () => adminApi.vehicleSeatsGenerate(vehicleId!, { prefix: spec.prefix.trim(), count: Number(spec.count), class: spec.class }),
    onSuccess: (result: { data?: { created?: number } }) => {
      toast(`${result?.data?.created ?? 0} kursi baru dibuat.`, 'success');
      queryClient.invalidateQueries({ queryKey: ['vehicle-seats', vehicleId] });
    },
    onError: (error) => toast(extractApiError(error, 'Gagal membuat kursi.'), 'error'),
  });

  const toggleMutation = useMutation({
    mutationFn: ({ seatId, isActive }: { seatId: number; isActive: boolean }) => adminApi.vehicleSeatUpdate(vehicleId!, seatId, { is_active: isActive }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['vehicle-seats', vehicleId] }),
    onError: (error) => toast(extractApiError(error, 'Gagal memperbarui kursi.'), 'error'),
  });

  return (
    <div className="space-y-6">
      <PageHeader title="Kursi Kendaraan" description="Buat denah kursi armada secara massal. Kursi inilah yang tampil di pemilihan kursi pelanggan." />
      ...
      <AppCard>
        ...
        {vehicleId ? (
          <form className="mt-5 grid gap-3 sm:grid-cols-[auto_auto_auto_auto] sm:items-end" onSubmit={(e) => { e.preventDefault(); generateMutation.mutate(); }}>
            <label className="block"><span className="mb-1.5 block text-sm font-bold">Prefix</span><input value={spec.prefix} onChange={(e) => setSpec({ ...spec, prefix: e.target.value })} maxLength={4} className={`${input} w-24`} /></label>
            <label className="block"><span className="mb-1.5 block text-sm font-bold">Jumlah</span><input type="number" min={1} max={60} value={spec.count} onChange={(e) => setSpec({ ...spec, count: e.target.value })} className={`${input} w-28`} /></label>
            <label className="block"><span className="mb-1.5 block text-sm font-bold">Kelas</span>
              <select value={spec.class} onChange={(e) => setSpec({ ...spec, class: e.target.value })} className={input}>
                <option value="regular">Regular</option><option value="vip">VIP</option><option value="executive">Executive</option>
              </select>
            </label>
            <button type="submit" disabled={generateMutation.isPending} className={btnPrimary}>{generateMutation.isPending ? <Loader2 size={15} className="animate-spin" /> : <Plus size={15} />} Generate kursi</button>
          </form>
        ) : null}
      </AppCard>
      ...
      {vehicleId ? (
        seatsQuery.isLoading ? (
          <Skeleton className="h-40" />
        ) : (seatsQuery.data?.length ?? 0) === 0 ? (
          <EmptyState title="Belum ada kursi" description="Gunakan generator di atas untuk membuat denah kursi." />
        ) : (
          <AppCard>
            <SectionHeader title={`${seatsQuery.data!.length} kursi`} description="Klik kursi untuk mengaktifkan/menonaktifkan." />
            <div className="mt-5 flex flex-wrap gap-2">
              {seatsQuery.data!.map((seat) => (
                <button
                  key={seat.id}
                  onClick={() => toggleMutation.mutate({ seatId: seat.id, isActive: !seat.is_active })}
                  disabled={toggleMutation.isPending}
                  className={`min-h-11 min-w-14 rounded-xl border px-3 text-xs font-extrabold transition ${seat.is_active ? 'border-primary bg-primary/10 text-primary' : 'border-slate-200 bg-slate-100 text-slate-400 line-through dark:border-slate-800 dark:bg-slate-800'}`}
                  title={seat.is_active ? 'Aktif — klik untuk nonaktifkan' : 'Nonaktif — klik untuk aktifkan'}
                >
                  {seat.seat_number}
                </button>
              ))}
