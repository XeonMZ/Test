...
export type BookingTracking = {
  booking: { uuid: string; code: string; status: string };
  schedule: {
...
export const adminApi = {
  ...
  driverUpdate: async (id: number, payload: { status?: string; license_number?: string }) => (await http.patch(`/admin/drivers/${id}`, payload)).data,
  ...
  vehicleSeats: async (vehicleId: number) => (await http.get<{ data: Array<{ id: number; seat_number: string; class: string; is_active: boolean }> }>(`/admin/manage/vehicles/${vehicleId}/seats`)).data.data,
  vehicleSeatsGenerate: async (vehicleId: number, payload: { prefix: string; count: number; class?: string }) => (await http.post(`/admin/manage/vehicles/${vehicleId}/seats/generate`, payload)).data,
  vehicleSeatUpdate: async (vehicleId: number, seatId: number, payload: { is_active?: boolean; class?: string }) => (await http.patch(`/admin/manage/vehicles/${vehicleId}/seats/${seatId}`, payload)).data,
  vehicleCreate: async (payload: { plate_number: string; code: string; brand: string; status: string }) => (await http.post('/admin/vehicles', payload)).data,
};
