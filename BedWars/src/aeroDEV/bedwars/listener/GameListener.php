...
$world = $position->getWorld();
        $wool = VanillaBlocks::WOOL()->setColor($session->getTeam()->getDyeColor());
        $scheduler = BedWars::getInstance()->getScheduler();

        $placementsByTick = [];

        // badan tower (dibangun layer per tick agar ringan)
        for($y = 0; $y <= 4; $y++) {
            $delay = $y + 1;
            for($x = -1; $x <= 1; $x++) {
                for($z = -1; $z <= 1; $z++) {
                    if($y === 0 || $x === -1 || $x === 1 || $z === -1 || $z === 1) {
                        $placementsByTick[$delay][] = [$base->add($x, $y, $z), clone $wool];
                    }
                }
            }
        }

        // lubang depan (bukan pintu)
        $placementsByTick[6][] = [$base->add(0, 1, -1), VanillaBlocks::AIR()];
        $placementsByTick[6][] = [$base->add(0, 2, -1), VanillaBlocks::AIR()];

        // battlement atas
        foreach([[-1, 5, -1], [0, 5, -1], [1, 5, -1], [-1, 5, 0], [1, 5, 0], [-1, 5, 1], [0, 5, 1], [1, 5, 1]] as [$x, $y, $z]) {
            $placementsByTick[7][] = [$base->add($x, $y, $z), clone $wool];
        }

        // ladder ke battlement (di dalam tower)
        for($y = 1; $y <= 4; $y++) {
            $placementsByTick[7][] = [$base->add(0, $y, 0), VanillaBlocks::LADDER()->setFacing(Facing::NORTH)];
        }

        foreach($placementsByTick as $delay => $entries) {
            $scheduler->scheduleDelayedTask(new ClosureTask(function() use ($world, $session, $entries): void {
                foreach($entries as [$pos, $block]) {
    $world->setBlock($pos, $block);

    if(!$block instanceof Air) {
        $session->getGame()->addBlock(new Position(
            $pos->getX(),
            $pos->getY(),
            $pos->getZ(),
            $world
