...
final class QuickBuyManager {
    ...
    public static function getLayout(Session $session): array {
        if(!ConfigGetter::isQuickBuyEnabled()) {
    ...
    public static function toggleProduct(Session $session, ItemProduct $product): void {
        if(!ConfigGetter::isQuickBuyEnabled()) {
            $session->message("{RED}Quick Buy is disabled in config.");
            return;
        }
    ...
    private static function getDefaultLayout(): array {
        $configured = ConfigGetter::getQuickBuyDefaultLayout();
        return $configured !== [] ? $configured : self::DEFAULT_LAYOUT;
    }
}
