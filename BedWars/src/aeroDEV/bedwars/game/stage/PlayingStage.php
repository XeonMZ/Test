...
use aeroDEV\bedwars\game\event\presets\UpgradeGeneratorsTierEvent;
use aeroDEV\bedwars\game\generator\GeneratorType;
use aeroDEV\bedwars\game\generator\Tier;
use aeroDEV\bedwars\session\scoreboard\GameScoreboard;
...
class PlayingStage extends Stage {
    ...
    public function onQuit(Session $session): void {
        ...
        if($session->hasTeam()) {
            ...
            if(!$team->isAlive()) {
                $this->game->broadcastMessage("§l§4BED§bWARS §f>> §7[INFO] " . $team->getColoredName() . " §fTeam tereliminasi!");
            }
        }
