...
class BedWarsCommand extends Command implements PluginOwned {
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): void {
        ...
        if(strtolower($args[0] ?? "") === "setboard") {
            $variable = FloatingLeaderboardManager::normalizeVariable($args[1] ?? "");
            $mode = FloatingLeaderboardManager::normalizeMode($args[2] ?? "");

            if($variable === null || $mode === null) {
                $sender->sendMessage("§cUsage: /bedwars setBoard <Wins/Kills> <Solos/Duos/Squad/Total>");
                return;
            }

            BedWars::getInstance()->getFloatingLeaderboardManager()->setBoardFromPlayer($sender, $variable, $mode);
            $sender->sendMessage("§aLeaderboard set: " . strtoupper($variable) . " - " . strtoupper($mode));
            return;
        }
