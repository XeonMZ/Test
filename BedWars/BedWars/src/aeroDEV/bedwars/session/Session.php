...
namespace aeroDEV\bedwars\session;
...
use aeroDEV\bedwars\session\scoreboard\LobbyScoreboard;
use aeroDEV\bedwars\session\scoreboard\Scoreboard;
use aeroDEV\bedwars\session\settings\GameSettings;
use aeroDEV\bedwars\session\settings\SpectatorSettings;
use aeroDEV\bedwars\session\setup\MapSetup;
...
class Session {
    ...
    public function addKill(): void {
        ...
        $this->leaderboard_stats[$mode]["kills"] = ($this->leaderboard_stats[$mode]["kills"] ?? 0) + 1;
        $this->setKills($this->getTotalKills());
    }
    ...
    public function addWin(): void {
        $mode = $this->getCurrentModeKey();
        $this->leaderboard_stats[$mode]["wins"] = ($this->leaderboard_stats[$mode]["wins"] ?? 0) + 1;
        $this->setWins($this->getTotalWins());
    }


    public function getLeaderboardStats(): array {
        return $this->leaderboard_stats;
    }

    public function setLeaderboardStats(array $stats): void {
        foreach(["solo", "duo", "squad"] as $mode) {
            $mode_data = $stats[$mode] ?? [];
            $this->leaderboard_stats[$mode] = [
                "kills" => (int) ($mode_data["kills"] ?? 0),
                "wins" => (int) ($mode_data["wins"] ?? 0)
            ];
        }
        $this->kills = $this->getTotalKills();
        $this->wins = $this->getTotalWins();
    }

    public function getTotalKills(): int {
        return (int) (($this->leaderboard_stats["solo"]["kills"] ?? 0) + ($this->leaderboard_stats["duo"]["kills"] ?? 0) + ($this->leaderboard_stats["squad"]["kills"] ?? 0));
    }

    public function getTotalWins(): int {
        return (int) (($this->leaderboard_stats["solo"]["wins"] ?? 0) + ($this->leaderboard_stats["duo"]["wins"] ?? 0) + ($this->leaderboard_stats["squad"]["wins"] ?? 0));
    }

    private function getCurrentModeKey(): string {
        $players_per_team = $this->game?->getMap()->getPlayersPerTeam() ?? 4;
    ...
    public function giveWaitingItems(): void {
        $this->clearAllInventories();
        $inventory = $this->player->getInventory();
        $inventory->setItem(0, BedwarsItems::VOTE_MAP()->asItem());
        $inventory->setItem(4, BedwarsItems::SETTINGS()->asItem());
        $inventory->setItem(8, BedwarsItems::LEAVE_GAME()->asItem());
    }
    ...
    public function kill(int $cause): void {
        ...
        if($killer_session !== null) {
            ...
            if($cause === EntityDamageEvent::CAUSE_ENTITY_ATTACK) {
                $this->game->broadcastMessage("§l§4BED§bWARS §f>> §7[INFO] " . $session_username . " §7dibunuh oleh " . $killer_username . "§7.");
            } elseif($cause === EntityDamageEvent::CAUSE_VOID) {
                $this->game->broadcastMessage("§l§4BED§bWARS §f>> §7[INFO] " . $session_username . " §7terjatuh ke void oleh " . $killer_username . "§7.");
            }
        ...
        if($cause === EntityDamageEvent::CAUSE_VOID and $killer_session === null) {
            $this->game->broadcastMessage("§l§4BED§bWARS §f>> §7[INFO] " . $session_username . " §7jatuh ke void.");
        }
    ...
    private function vanish(): void {
        ...
        foreach($this->game->getPlayers() as $session) {
            ...
            $session->sendDataPacket(MobArmorEquipmentPacket::create($id, $item, $item, $item, $item, $item));
        }
    }
