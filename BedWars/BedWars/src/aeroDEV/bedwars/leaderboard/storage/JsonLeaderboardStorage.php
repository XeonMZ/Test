...
class JsonLeaderboardStorage implements LeaderboardStorageContract {
    ...
    public function getTop(string $variable, string $mode, int $limit = 10): array {
        ...
        foreach(glob($users_dir . "*.json") as $file) {
            ...
            if($normalized_mode === "total" && is_array($stats_by_mode)) {
                $value = (int) (($stats_by_mode["solo"][$variable] ?? 0) + ($stats_by_mode["duo"][$variable] ?? 0) + ($stats_by_mode["squad"][$variable] ?? 0));
            } elseif(is_array($stats_by_mode) && isset($stats_by_mode[$normalized_mode][$variable])) {
                $value = (int) $stats_by_mode[$normalized_mode][$variable];
            } else {
                $value = (int) $config->get($variable, 0); // fallback legacy wins/kills
            }
