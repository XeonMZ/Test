...
final class InstallerService
{
    public function installed(): bool
    {
        ...
        // ->value() returns the RAW column (bypasses the model's array cast),
        // so it can be a JSON string like "true"/"false", or 1/0. Normalize
        // robustly — a naive (bool) cast treats the string "false" as true.
        $raw = SystemSetting::query()->where('key', 'installer.locked')->value('value');
        if (is_bool($raw)) {
            return $raw;
        }
    ...
    public function install(array $payload): array
    {
        ...
        // Self-heal a fresh deploy: if migrations have not been run yet, run
        // them now so the install does not fail with "table not found". This
        // makes the wizard work on hosts where the release step could not run
        // `php artisan migrate` (e.g. no shell access).
    ...
    private function bootstrapSettings(array $payload): array
    {
        ...
        return [
            ...
            'notification.sender_email' => data_get($payload, 'mail.sender_email'),
