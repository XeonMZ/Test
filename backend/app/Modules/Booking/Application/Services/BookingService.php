...
final class BookingService
{
    ...
    public function createBooking(array $data): Booking
    {
        return $this->db->transaction(function () use ($data): Booking {
            ...
            $baseAmount = (float) ($data['amount'] ?? $schedule->base_fare * max(1, count($seatIds)));

            // #7 Apply promo code if provided and valid.
            $discount = 0.0;
            $promoCode = null;
            if (! empty($data['promo_code'])) {
                $promo = \App\Models\Promo::query()
                    ...
                    ->first();
                if ($promo) {
                    // #9 One lifetime redemption per customer, enforced by the
                    // UNIQUE(promo_id, customer_id) constraint. Insert-first
                    // (not check-then-insert) so two concurrent bookings with
                    // the same code cannot both redeem it.
                    try {
                        ...
                        throw new \InvalidArgumentException('Kode promo ini sudah pernah kamu gunakan dan hanya berlaku satu kali per akun.');
                    }
                    $discount = min($baseAmount, (float) $promo->amount);
            ...
            \App\Models\VehicleSeat::query()->whereIn('id', $seatIds)->orderBy('id')->lockForUpdate()->get();
            $this->validator->assertSeatsAvailable($schedule, $seatIds, $now);
            $booking = Booking::create([
                'code' => 'BKG-'.Str::upper(Str::random(10)),
                'schedule_id' => $schedule->id,
                'customer_id' => $customer->id,
                'status' => BookingStatus::SeatLocked->value,
                'amount' => $amount,
                ...
                'drop_label' => $data['drop_label'] ?? null,
            ...
            if ($promoCode !== null) {
                \App\Models\PromoRedemption::query()
                    ->where('promo_id', \App\Models\Promo::where('code', $promoCode)->value('id'))
    ...
    public function lockSeat(string $bookingUuid, array $seatIds): Booking
    {
        return $this->db->transaction(function () use ($bookingUuid, $seatIds): Booking {
            ...
            // Row-lock the physical seats first: two concurrent lockSeat calls for
            // the same seat now serialize here instead of both passing the
            // availability check below and double-booking the seat.
            \App\Models\VehicleSeat::query()->whereIn('id', array_map('intval', $seatIds))->orderBy('id')->lockForUpdate()->get();
