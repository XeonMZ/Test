...
final class CheckInService
{
    ...
    public function checkIn(string $qrPayload, string $driverUuid, ?float $latitude, ?float $longitude): PassengerCheckIn
    {
        ...
        return $this->completeCheckIn($ticket, $driverUuid, $latitude, $longitude);
    }

    /**
     * Manual check-in by human-readable ticket number, used when the QR is
     * unreadable. The ticket number is unique and non-guessable enough for
     * counter use; access is still restricted to driver/admin/owner roles.
     */
    public function checkInByCode(string $ticketNumber, string $driverUuid, ?float $latitude, ?float $longitude): PassengerCheckIn
    {
        $ticket = TicketModel::with(['booking','trip'])->where('ticket_number', $ticketNumber)->lockForUpdate()->firstOrFail();
        return $this->completeCheckIn($ticket, $driverUuid, $latitude, $longitude);
    }

    private function completeCheckIn(TicketModel $ticket, string $driverUuid, ?float $latitude, ?float $longitude): PassengerCheckIn
    {
