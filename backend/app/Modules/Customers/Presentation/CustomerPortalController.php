...
final class CustomerPortalController extends Controller
{
    ...
    public function validatePromo(Request $request): JsonResponse
    {
        ...
        // #9 A promo is a one-lifetime-use voucher per account.
        $customerId = $request->user()?->customer?->id;
        $alreadyRedeemed = $customerId !== null && \App\Models\PromoRedemption::query()
            ->where('promo_id', $promo->id)
            ->where('customer_id', $customerId)
            ->exists();
        if ($alreadyRedeemed) {
            return response()->json(['success' => false, 'message' => 'Kode promo ini sudah pernah kamu gunakan dan hanya berlaku satu kali per akun.', 'data' => ['valid' => false]], 200);
        }
