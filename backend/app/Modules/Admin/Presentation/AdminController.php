...
final class AdminController extends Controller
{
    ...
    /**
     * #5 Create notifications. Reachable only through the /admin group
     * (role:admin,owner middleware) — drivers/customers can solely read their
     * own inbox. Targets one user, one role, or everyone; rows are inserted in
     * chunks so a broadcast to thousands of users stays memory-safe.
     */
    public function notificationStore(Request $request): JsonResponse
    {
        ...
        $recipients = match ($data['target']) {
            'user' => \App\Models\User::query()->whereKey($data['user_id']),
            'role' => \App\Models\User::query()->where('role', $data['role'])->where('is_active', true),
            default => \App\Models\User::query()->where('is_active', true),
        };

        $created = 0;
        $recipients->select('id')->chunkById(500, function ($users) use ($data, $type, &$created): void {
            $now = now();
        ...
        $this->log($request, 'notification.created', 'Notification', ['target' => $data['target'], 'count' => $created]);
