...
final class AdminController extends Controller
{
    ...
    /**
     * Delete a customer (owner only). Blocked while active bookings exist —
     * history stays intact via soft delete; the login account is deactivated
     * and its tokens revoked.
     */
    public function customerDestroy(Request $request, string $id): JsonResponse
    {
        abort_unless($request->user()?->role === 'owner', 403, 'Hanya owner yang dapat menghapus customer.');
        $customer = Customer::with('user')->findOrFail($id);
        $hasActive = $customer->bookings()->whereIn('status', ['seat_locked', 'waiting_payment', 'paid', 'ticket_generated'])->exists();
        abort_if($hasActive, 422, 'Customer masih memiliki booking aktif. Selesaikan atau batalkan booking terlebih dahulu.');

        $before = ['name' => $customer->user?->name, 'email' => $customer->user?->email];
        $customer->user?->forceFill(['is_active' => false])->save();
        $customer->user?->tokens()->delete();
        $customer->delete();
        $this->log($request, 'customer.deleted', 'Customer', ['id' => $customer->id, 'before' => $before]);
    ...
    // ----- Promo management (full CRUD, admin & owner) -----
    public function promoIndex(Request $request): JsonResponse
    {
    ...
    {
        $data = $request->validate([
            'code' => 'required|string|max:32|alpha_dash|unique:promos,code',
        ...
        $this->log($request, 'promo.created', 'Promo', ['id' => $promo->id, 'code' => $promo->code]);
        return response()->json(['data' => $promo], 201);
    }

    public function promoUpdate(Request $request, string $id): JsonResponse
    {
        $promo = \App\Models\Promo::findOrFail($id);
        $data = $request->validate([
            'code' => 'sometimes|string|max:32|alpha_dash|unique:promos,code,'.$promo->id,
        ...
        $before = $promo->only(array_keys($data));
        $promo->update($data);
        $this->log($request, 'promo.updated', 'Promo', ['id' => $promo->id, 'before' => $before, 'after' => $promo->only(array_keys($data))]);
        return response()->json(['data' => $promo->fresh()]);
    }

    public function promoDestroy(Request $request, string $id): JsonResponse
    {
        $promo = \App\Models\Promo::findOrFail($id);
        $before = $promo->only(['code', 'name', 'amount']);
        $promo->delete();
        $this->log($request, 'promo.deleted', 'Promo', ['id' => (int) $id, 'before' => $before]);
        return response()->json(['data' => ['deleted' => true]]);
    }
    ...
    public function bookingSchedules(Request $request): JsonResponse
    {
        $q = $this->scheduleOverviewQuery($request)
            ->addSelect(['bookings_count' => DB::table('bookings as bc')
                ->whereColumn('bc.schedule_id', 'schedules.id')
    ...
    {
        // NOTE: select() must come BEFORE addSelect() — calling select() later
        // resets the column list and silently drops the aggregate subqueries
        // (root cause of the broken schedule overview).
        $q = Schedule::query()
            ->select('schedules.*')
        ...
        return $q->orderByDesc('departure_at');
    }
