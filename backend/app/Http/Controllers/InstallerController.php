...
final class InstallerController extends Controller
{
    ...
    {
        // Never 500 here — the wizard must always be able to render, even on a
        // freshly-deployed environment where the database or migrations are not
        // ready yet. Any failure degrades to installed=false + a diagnostic.
        try {
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            ...
            report($e);
            return response()->json(['message' => 'Instalasi gagal saat menulis ke database: '.$e->getMessage()], 422);
        } catch (\Throwable $e) {
            report($e);
            return response()->json(['message' => 'Instalasi gagal: '.$e->getMessage()], 422);
        }
    }
}
