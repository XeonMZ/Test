#!/usr/bin/env bash
# =====================================================================
# SJT backend release/start script for any container or VPS host.
#
...
# Ensure Laravel writable skeleton exists (empty dirs can be dropped by git/zip).
mkdir -p storage/app/public storage/app/private storage/framework/cache/data storage/framework/sessions storage/framework/testing storage/framework/views storage/logs bootstrap/cache

echo "==> SJT backend booting…"

# 1. APP_KEY — required for encryption. Prefer setting APP_KEY
#    in your host's environment (stable across deploys). If absent, generate one so the app
#    doesn't hard-fail; note this changes on each deploy if not persisted.
if [ -z "${APP_KEY:-}" ]; then
  echo "==> APP_KEY not set — generating a temporary key (set APP_KEY in your environment to persist)."
...
#    Toggle off by setting SEED_ON_DEPLOY=false if you ever want to.
...
# 5. Serve. PORT is provided by the host (defaults to 8000).
